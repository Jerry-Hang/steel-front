...
### 红蓝阵营不对称：**静态几何全对称，效应本身仍是边缘的**（2026-09-14 收束）

**实验（8 轮 × 2 臂）**：baseline 均值 **+16.5**、swapped 均值 **−6.3**，臂间差 22.8，
臂内极差均值 22.5 ⇒ **勉强超过噪声，但两组分布【重叠】**（baseline 最低 +6，swapped 最高 +6）。
...
**⇒ 这一步否证的价值**：掩体分布、出生点可站立性、出生几何配对 —— **这三类解释可以划掉了**，
而且每条都由一个**留在仓里的测试**钉死（`cargo test --nocapture` 可复现），不是靠推理。

**⚠️ 但必须同时承认**：**效应本身也没有被确证。**
22.8 vs 22.5 是"勉强超过"，两组分布**在 +6 处重叠**。
**最简约的读法是：这可能仍是噪声，8 轮不足以判定。**
⇒ **下一步若要继续，应当先加样本（≥20 轮/臂）而不是继续找机制** ——
在没有确认效应存在之前找成因，就是在给噪声编故事（这与本文档教训 24 是同一件事）。

**若要继续找机制，按性价比排序的剩余候选**（都属运行时/动态，静态已排除）：
1. **玩家在原点**：恒为 `Team::Blue`、无敌。**验证法**：把玩家挪到 `(+40, 0)`，看效应是否跟着移动。
2. **NPC 创建顺序**：红方先建（`side == 0`）⇒ `next_npc_id` 更低。**这解释不了"跟着半场走"**，但值得一并排除。
3. ~~数组下标与空间方位的耦合~~ **已查并排除**：`role_for(i, 1, …)` 两侧同 `i` ⇒ 角色镜像对称；
   `radius` 的 `((i*7+side)%5)` 抖动虽依赖 `side`，但实测只造成 **0.6°** 的角度均值差与 ±1 的人数差。
