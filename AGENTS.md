...
1. **其余 23 件道具仍带着焊接造成的顶点色错位**（2026-09-17 发现，**未修**）。
   `tools/blender/weld_props.py` 以前用**顶点下标**去读 **CORNER domain** 的 `COLOR_0`
   ⇒ 实测 tree_oak **2725 个顶点里 2284 个（84%）拿到的不是自己的颜色**。
   工具已修、`tree_oak` 已重生成重焊（棕色顶点 218→122、y 上限 7.18m→5.40m）。
   **但 `commit 82a2306` 那次一次性焊了全部 24 件**，其余 23 件的原始颜色信息在已入库的
   焊接结果里**已经丢了** —— 只能从生成器重跑。⚠ `assets/props` 里 6 个建筑模块归
   `build_city_kit.py` 管、其余归 `gen_props.py`，**重生成前先确认每件的主人**，
   并且必须 `preview_glb.py` 看过再入库（预览图对颜色不可信，最终判断要在引擎里做）。
   **⇒ 判据：重跑后逐件比对顶点数与尺寸契约，对不上就是 generator 已经漂移。**
