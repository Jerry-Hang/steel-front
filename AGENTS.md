...
- 验收：**`cargo test --release` 457 全绿（mesh 路径下）**；实机截图 `screenshots/mesh1_a.png` / `nosway_a.png`；无 device-lost、无 VUID 报错、`cap_safe.ps1` 确认无残留进程。
  ...
  - **⚠ 枪朝向：缺陷真实存在，且我此前两次下错结论。当前状态 = 未解决。**
    - **实测现象（三张截图交叉确认）**：`AK-12M`（走 `Y-long` 分支，用旧 `ak12.glb`）**朝向正确**——枪口指向左上方消失点；
      `asval` / `pkm`（走 `IDENTITY` 分支，用规范化后的新 GLB）**横躺、枪口朝屏幕右**，偏转约 90°。
      所以缺陷是真的，且**与 align 分支相关**：`Y-long` 对、`IDENTITY` 错。
    - **探针结论不可信，别再拿它当证据**：`gun-orient` 探针曾报
      `asval 世界跨度 前向=0.676m 右向=0.038m → 沿视线（正确）`，与截图直接矛盾。
      已核对 `camera.rs:143-151` 的 `forward()`/`right()` 定义**没有写反**（forward=(0,0,-1) 时 right=+X），
      所以问题在探针本身：它用 `fp_gun_matrix()` 组合顶点，而**实际绘制走的变换组合与它不同**
      （线索：`weapon_data.rs` 有测试 `gun_fp_viewspace_no_clip`，说明存在一条"把枪模顶点烘进视空间"的路径，
      若绘制走的是那条，则探针是在已变换的顶点上又乘一次 `fp_gun_matrix`，测出来的是无意义值）。
    - **下一轮的入口**：先确定 GLB 枪模顶点从 `load_gun_glb` 到 `cmd_draw_indexed` 之间**被乘过哪几个矩阵**
      （查 `set_first_person_gun_mesh` / `set_first_person_gun_model` 与 `fp_gun_pre` 的实际构成），
      再决定是修 `IDENTITY` 分支的旋转、还是修探针。**不要用肉眼继续判断朝向**——我已因此错过两次。
    - **方法论修正**：此前我写过的"透视 ≠ 朝向，肉眼会被 0.6m 锚距骗"这条**用反了方向**——本次三张图里
      AK-12M 与 AS Val 在同一锚距下呈现明显不同，说明差异是真实的几何旋转差异，不是透视错觉。
      正确教训是：**探针本身也可能是错的**（它可能测的不是绘制用的那条变换链）。
      三维判断要**两条独立证据互相印证**（例如"探针数字"与"两把枪同机位对照截图"），
      任一单独成立都不足以结案——我这次就是信了探针、否掉了正确的肉眼观察。
    - 结论：**13 把规范化枪模已正确安装、几何与亮度均正常，但 `IDENTITY` 分支的朝向仍是错的（未修）**。
      探针保留为诊断工具，但**其输出在查明绘制变换链之前不可作为结论依据**。
