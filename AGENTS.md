...
### [2026-09-11] 交接：围墙/街树/花坛三处已修并实机验证；**但 ① 只压住了症状，病根是"盒子的顶面没上屏"**

- 日期：2026-09-11
- 发起方：Qwen（接回 09-10 那次被中断的会话）
- 接收方：下一会话 AI
- 交接类型：迭代结束 + **一条推翻本轮自己 ① 结论的新发现（上手先读这条）**
- 验收：`cargo test --release` **459 passed / 0 failed**、0 警告；commits `1de7ffd`、`d51afab`、`501b579`（均已 push）
- 本轮方法：每条都走「查代码 → 断言/截图取证 → 改 → 同机位复拍」。中途**三次推翻自己的判断**（见 ①⑤⑦），都记在这里。

  **① ✅ 隔离带"纸片化"结案：剖面写反了（压顶比墩身宽），已修；剩下的观感问题是配色/皮肤归属。**
  - **改前现象**（`green_spawn2_b.png` 放大）：一道亮平板横在路中间、两侧各一道矮墙、板下方还
    露着路面 → 读作"一只没有盖的浅盘浮在沥青上"。
  - **根因（硬错误，与投影无关）**：`street_furniture` 把压顶做成**比墩身宽 0.30m**（0.9 → 1.2）。
    真泽西护栏是**底宽顶窄**，写反了。俯视 + 掠射角下，那道 0.15m/边的出挑把"板"和"墙"在视觉上
    切离开来，于是读成桌板。**已改三段收分**（下 1.00 / 上 0.62 / 顶带 0.82，总高仍 1.20、
    反光柱不变），受光水平面 7.6→4.9m²，`median_taper_b.png` 已读作实体矮墙。
  - **✅ A（"压顶顶面没上屏"）已由源码判死：不要去动 shader。**
    `src/engine/meshgen.rs:176` 是
    ```rust
    let faces: [(glam::Vec3, glam::Vec3, glam::Vec3, glam::Vec3, glam::Vec3); 6] = [ ... ];
    for (n, p0, p1, p2, p3) in faces { ... }
    ```
    **六个面无条件全发射、所有盒子共用同一套几何**（`Shape` 只改着色标签，不改发射哪几个面）。
    所以**代码里不存在"某种 shape 漏掉 +Y 面"这条路径**。这条与读图无关，是本轮 ① 上唯一硬的事实。
  - **🔴 关于"浅盘/桌板"的机制，我没有结论，并且我在这条上翻了至少四次——下一轮别接我的判断。**
    我先后判过「顶面没上屏」→「实心盒子的正常掠射投影」→「那两道鳍是几十米外的柱廊、和护栏无关」
    →「改后的台阶剖面本来就该长这样」。**每一次都只凭一张局部裁剪 + 纯推理，没有一次独立验证。**
    **而且我至少一次把 before/after 搞混了**：`#250bfec9fa3a` 是对 `green_spawn2_b.png`（**改前**）
    的裁剪，可它呈现的轮廓（宽板在下、窄墙从板上立起、板向外超出墙）是**底宽顶窄**才有的样子，
    而改前那版是**压顶比墩身宽**（顶宽底窄）——**两者互相矛盾**。
    这说明我对这几张图的归属/ provenance 至少错了一次，因此**基于它们的全部读图结论一并作废**。
    **下一轮若要继续 ①，第一件事不是看图，是重拍一组带标签的对照**：
    同一机位、明确记下"这一张对应哪一个 commit 的 exe"，并且**整帧判读、不要拿裁剪推机制**。
  - **① 可以确定、且不依赖任何读图的两条**：
    1. **硬错误一处，已修**（`d51afab`）：压顶做成**比墩身宽 0.30m**（0.9→1.2），
       而真泽西护栏是底宽顶窄。这条是**代码与常识层面的错**，与画面怎么读无关，改它总是对的。
    2. **shader/几何不需要动**（上面 meshgen 那条已排除"少画一个面"）。
       若改完仍显"纸片"，方向只剩**配色/皮肤归属**（大面积近白受光水平面 + 平着色无深度线索），
       那是 `city.rs` 的 tint/Shape 选择，**风险低、不用碰管线**。
  - ℹ 旧账一并纠正：09-09"未视觉验证"的 `shop_block` 骑楼棚、`spvchk_b.png`"白色交叉薄壁"、
    09-09/09-10 未归因的"悬浮走廊"——**都不要挂在"顶面没上屏"上（源码已排除该路径）**。
    "白色交叉薄壁"就是中央隔离带（09-10 结案 + 本轮改了剖面）；另两件**尚未归因，别替我猜**。
  - **ℹ 关于"改前像桌板、改后像矮墙"这句观感对比：不要引用它作为任何证据。**
    本轮我把 `green_spawn2_b.png` / `median_taper_b.png` 的 before/after 归属搞混过至少一次，
    而这两张图恰好是那句对比的依据。**改动本身（底宽顶窄）是对的，不需要画面来为它背书。**

  **② ✅ 边界围墙两处算术错，已修（`1de7ffd`）并实机验证。**
  - **四角各差 22.5m**：`perimeter` 铺 7 段 × 55m = 385m 到 430m 的墙线上，四角是开口的，
    能直接走出城市。旧测试 `perimeter_has_no_unintended_gaps` 只查"相邻段之间有无豁口"，
    而段间确实没有豁口 → **一路绿灯**。教训写进测试注释：**"没有豁口" ≠ "闭合"。**
    现在首末段外沿直接铺到 ±CITY_WALL，测试扩成四面都测 + 新增"必须铺到 ±CITY_WALL"断言。
  - **四个大门全被墙填死**：大门分支把 `(seg - gate) * 0.5` 当成**半宽**传给 `pw`，
    每片墙身实宽 43m、两片在中央重叠 9.5m → 12m 门洞根本不存在，门柱整根埋在墙里，
    只有旗杆和灯头从墙头伸出来。新增 `gates_are_passable` 钉死。
  - 取证：改前两条断言都 FAILED（`side 0 只铺到 [-192.5, 192.5]` / `大门 (0,-215) 被墙体堵死`）；
    改后 `gate_out_b.png`（站在门外向内看，能透过门洞看见对侧建筑）与 `corner_a2_b.png`（墙角连续）。

  **③ ✅ 街树没有树干，已修（`d51afab`）。** `block_edges` 的四角树池原来直接摆一颗
  1.7m 叶子球（底 −0.05、顶 1.30）——平着色下就是压在人行道上的绿色巨石（`green_oak_b.png` 左下）。
  改为调用 `tree()`（有干有冠，资产在时走 `tree_oak` GLB）。dump 复核：8 处裸球已变成
  0.30m 细碰撞柱 + GLB 树，`green_oak2_b.png`/`green_spawn2_b.png` 里已是成排街树。
  ⚠ **`tree_oak` 资产本身是好的**（有干有冠），09-10 记的"tree_oak 无干/摆在路上"是**误诊**。

  **④ ✅ 广场花坛同类错误，已修（`501b579`）。** `plaza()` 手搓一颗 **3.4m 宽 × 1.4m 高**的扁球
  （宽高比 0.41）当灌木 → 一块压在石框上的绿石头。改走 `bush()`（宽高比 ~0.77）。
  **规律：把球压扁当植物，得到的永远是石头。** 以后加绿化一律走 `tree()`/`bush()`，不要手搓球。

  **⑤ ⚠ 我上一轮记的"⑤ 线索"是假的，作废。** 我当时拿 `#5c4c8e9e6fdd` 里
  左上 `AK-12M 风暴 … ammo: 1` 对上底部 `M1 Rifle AMMO 30/30 +90 | GRENADES 2`，
  判成"两套状态源不同步"。**错**：那些帧是 `RV3D_CAM` 下拍的，而 `main.rs::update()` 的
  cam_override 分支**直接 return、根本不跑 `game.update()`**，于是 `hud.weapon_name`/`ammo`
  永远停在 `ui.rs:499` 的**默认值**（正是 "M1 Rifle" / 30 / +90 / 2）。这是调试机位的副作用。
  - 代码侧初查**没有**发现明显缺陷：`active_weapon_key()` 与 rack 同源
    （rack 就是按 `ALL_WEAPONS` 全 35 条、同序构建的，索引空间一致），
    `gun_mesh_cache` 是 `Option<(String, GunMesh)>`、按 key 命中，切枪后 key 变了必然重建。
  - **下一轮要复现真缺陷，必须不带 `RV3D_CAM`**：正常开局 → 滚轮 / 数字键 / 指令切枪 →
    每切一次拍一张，比对枪模是否跟着换。别再用调试机位的截图判 ⑤。

  **⑥ ✅ 已修（`65bb2f2`）且本轮已实机确认。** 调试机位下 HUD 大号青色 FPS 恒 0 的问题，
  本轮所有 `RV3D_CAM` 截图左上角显示的是 107 / 129 而不是 0 —— 09-10 欠的那张取证图补上了。

  **⑦ 新的待查（本轮只取证，未动）：地面是大方格拼花，不是纹理噪声。**
  `#e58bdde3bc99` 下半屏看得很清楚：沥青是**一块块网格对齐、明暗交替的方形补丁**（约 10–15m 见方），
  每块内部颜色均匀、块与块之间硬切。**离散量化**的特征，不是连续噪声。
  - 已排除两个候选：`obstacle_material_tint` 的量化格点是 **1/8m**（太细，对不上 10m）；
    `procedural.rs` 的地面微细节 tile 是 **256px / 2m = 128 纹素每米**（也对不上 10m 周期）。
  - **下一步**：找地面 65536 实例场的**逐实例 tint**（renderer.rs 里生成 ground instance 的那段），
    看是不是按实例坐标做了 `floor(pos/N)` 级的哈希抖动、N≈10m。找到后把振幅压到 ~0.03 或改成连续噪声。

- 状态：②③ done（均已提交并 push、均有实机对比图）；① **partial**（症状已压住，根因待下一轮俯视定性）；
  ⑤ **未复现**（上一轮线索作废，需不带调试机位重测）；⑦ **open**（已取证、已排除两个候选）
- ⚠ **`501b579`（花坛改 bush()）只有测试绿，没有复拍验证。** 下一轮顺手用
  `RV3D_CAM=fly:-12,1.7,-9:90,0` 复拍一张，与 `green_oak_b.png` 对比：画面左右那两块
  "绿色巨石"应该变成两丛立体的灌木。若还在，说明 `bush()` 的比例也不够，别再猜第三次。

### [2026-09-10] 交接：**③ 的白墙查明了（中央隔离带）**；② 已修并提交；⑨ 的结论被推翻；**冒烟闸门当前是红的**

- 日期：2026-09-10
- 发起方：DeepSeek Harness（接手 09-09 被中断的那次 Qwen 会话）
- 接收方：下一会话 AI
- 交接类型：迭代结束 + **一条推翻前结论** + **一条新的红灯（上手先读）**
- 验收结果：`cargo test --release` **458 passed / 0 failed**、0 警告；冒烟 **FAIL**（原因见 ④，与本轮改动无关）

  **① 接手时的仓库状态（先说清楚中断留下了什么）。** 09-09 那次会话在 09-10 中午被中断，留下：
  - `src/main.rs` 有 **未提交** 的 HUD 修复（就是 09-09 交接的 ②），已写完、能编译，只是没提交、没截图；
  - `src/engine/city.rs` 的 mtime 被改过但内容与 HEAD 逐字节相同（改了又回退，没留痕迹）；
  - `scripts/_paper_dump.rs`（09-10 11:16，仓库里最新的文件）是给 ③ 写的取证工具，
    **从未编译运行过**（放在 `scripts/` 下不参与 crate 构建），所以 ③ 当时仍未查明；
  - 根目录 `main.rs` 是 `src/main.rs` 的过期副本（diff 只差那 6 行 HUD 修复，无独有内容）。
  - **已全部处置**：② 提交为 `65bb2f2`；`_paper_dump.rs` 与根 `main.rs` 等 37 个 9-02/9-10 诊断残留已删。

  **② ✅ ③ 结案：那坨白墙是 `city.rs::street_furniture` 的「中央隔离带」，不是广场柱廊、也不是骑楼棚。**
  - **生成点**：`city.rs:1146`，8 个坐标 `(±14,0)/(0,±14)/(±69,0)/(0,±69)`；每处三件——
    `Part::new(Building, x, z, 6.0, 0.9, UNDER_GROUND, 0.95, CONCRETE)`（墩身，`CONCRETE=[0.56,0.55,0.53]` **近白**）
    + `deco(Building, x, z, 6.3, 1.2, 0.95, 1.20, CONCRETE_DARK)`（**压顶，出挑 0.15/边**）
    + 3 根 `0.22` 反光柱（1.20→1.90）。
  - **判据是 dump 出来的几何 + 逐条对齐，不是看图猜**：出生点 18m 内的全部件里，
    只有这四组是「≥1m 高 + 近白 + 带压顶」的墙；`(±14,0)/(0,±14)` 四组正好在四个轴向、
    离出生点 14m，近直角交叉 —— 与 09-09 描述的「白色薄壁交叉结构」逐项吻合。
    另**排除了 GLB 道具**：出生点 40m 内 12 件道具全是 `mesh#21`，最近的在 20.6m（成对沿街），
    不在脚下。`screenshots/spvchk_b.png` 的原生分辨率放大图里能直接看到**墙身 + 更深色的出挑压顶**，
    与上面两件的尺寸/材质一一对应。
  - ⚠ **为什么 09-09 那一轮没找到**：当时给的筛选条件是「水平跨度 >15m、**薄轴 <0.5m**、
    **base >1m**」，而这四组是 **跨 6m、厚 0.9m、base≈0（`UNDER_GROUND` 落到 −0.05）**——
    三条全部落在筛选阈值的**另一侧**，被自己的过滤器挡掉了。**教训：筛选阈值要从"现象"反推，
    别从"我以为它该长什么样"来定。**
  - **是否算缺陷，留给下一轮定夺（本轮只定位，不改）**：几何本身自洽（压顶端正坐在墩身上、无悬空），
    问题在**摆位与配色**——出生点就是中央路口，四堵 1.2m 近白混凝土墙在四个轴向 11–17m 处把视野封死，
    平着色下读作"一坨白结构"。可选方向：中央那四组挪远/降矮/改深色，或只保留 `±69` 那四组。

  **③ ✅ ② 已修并提交（`65bb2f2`）。** `cam_override` 分支在 `return` 前补
  `self.game.hud.fps = self.last_fps as f32;`（`last_fps` 是本分支刚算出的 f64，`hud.fps` 是 f32，
  类型与 `game.rs` 既有写法一致）。**仍未补实机截图**——注意它只在 `RV3D_CAM` 下才显现。

  **④ 🔴 新的红灯：冒烟闸门在干净 HEAD 上就 FAIL（与 09-10 的改动无关，别误判成回归）。**
  - 现象（复现 3 次，含一组对照）：`VUID=0 panics=0`，但 **`kills=0` 且 `fps_min<120`**。
  - **对照组**：把 `assets/*.spv` 换回 HEAD 旧版再跑，**给出完全相同的失败模式**
    → 红灯笼与 .spv / HUD 两处改动**无关**。
  - 真因：游戏实际跑的是 **128v128 压力模式**，日志有
    `battle: 压力模式第 0 轮开战（红 128 vs 蓝 127+玩家，共 255 名 NPC）`。
    而**压力模式下玩家是无敌旁观者**（`game.rs:295`「压力模式远处红蓝互射不算（玩家无敌旁观）」），
    所以「瞄准+开火 → `kills>=1`」这条判据在该模式下**结构性不可能满足**——
    不是打不准，是规则不让杀。`fps_min` 同理：255 NPC 的负载低于 128 帧线。
  - 可疑面已收窄到**冒烟脚本这条调用链**：`run_gameplay_smoke.ps1:17` 写了
    `$env:RV3D_STRESS_AI = "0"`，但游戏进程里显然不是 "0"。已排除游戏侧——
    用**同一套** `Start-Process` 机制（含经 `powershell -File` 5.1 中转）直接启动，
    游戏**确实**进波次模式（`npcs=4/6/8`），说明 `main.rs`/`game.rs` 对环境变量的解析没问题。
    日志显示那次运行前 1 行还是 `enemies=8`（波次），下一行就被顶成 255。
  - **下一轮第一步**：查 `run_gameplay_smoke.ps1` → `gameplay_smoke_win.py` 之间是不是还有一处
    启动/重启游戏的路径没带上 `RV3D_STRESS_AI`（已确认 python 侧没有 `Popen`/`Start-Process`，
    所以更可能是环境块继承或残留进程复用）。**在它修好之前，别把冒烟 FAIL 当成代码回归。**

  **⑤ 另外两条顺手记录**：`smoke.log.err` 结尾那 5000+ 条 `device has been lost` 是
  **拆机噪声**（脚本 kill 与在飞帧竞争），不是崩溃——09-09 已记过，本轮再次踩到；
  以及**不要用 PowerShell 的 `Get-Content -Raw` + `Set-Content` 改这些源码文件**，
  中文会变乱码（本轮实测把 `city.rs` 弄坏一次，已 `git checkout` 复原）。
  根目录那个 `main.rs` 乱码副本大概就是这么来的。**改源码一律用编辑器工具，不要过 PowerShell 字符串。**

- 状态：①②③ 均 done（② ③ 已提交；① 的视觉验证仍欠）；④ blocked（已定位、待下一轮修闸门）

### [2026-09-09] 交接：**枪口朝向已根治——不是逐枪问题，是 `detect_axes` 把两端量反了**

- 日期：2026-09-09
- 发起方：当前会话 AI
- 接收方：下一会话 AI
- 交接类型：迭代结束（**推翻并取代 09-05/09-06/09-07 关于枪口的判断**）
- 交接内容：

  **① 真正的根因（一行）。** `tools/blender/prep_guns.py::detect_axes` 里
  ```python
  d_along = (co[:, L] - mn[L]) if sign > 0 else (mx[L] - co[:, L])   # 反了
  ```
  `sign > 0` 本该量"最大端"，写的却是**到最小端的距离**，于是 `ends[+1]` 是 −端的横截面、
  `ends[-1]` 是 +端的。`msign = 1 if ends[1] <= ends[-1] else -1` 因此恒等于**把粗的那端当枪口**。
  - `up` 走另一套 spine/tail 计算，**没受影响** → 现象正是"枪正立、但枪口朝后 180°"，与实机一致。
  - 复检调用同一个函数，错得完全一致 → **14/14 全报 `ok=True`、`barrel=Z+`**，报告从头到尾在说谎。
    这就是这个缺陷能活过一整轮验证的原因。

  **② 为什么前三次都判错**：都在**猜**"画面左右对应哪个轴"，没人去量。本轮先写了一个 66 行的
  标定程序 `tools/blender/camera_handedness.py`：红块放 +Y、蓝块放 −Y，用 `shoot()` **同一台**
  侧视相机渲一张图 → **实测画面右 = −Y = glTF +Z = 规范性要求的枪口端**。推导其实没错，
  但"实测一次"终结了三天的反复争论。**以后凡遇"画面哪边是哪个轴"，先跑这个脚本，别推。**

  **③ 证据是逐枪看图，不是推理**：修前 13 把可用枪（as_val/vss/pkm/pkp/vityaz/bizon/sv98/
  ak104/rpk16/mp443/osv96/saiga12/ash12）side 视图**枪托全在右 = 枪口全朝后**；
  修后**13 把枪口全在右**。`svd_63` 仍是含两把重叠枪身的产品宣传图，`install_guns.py` 照旧 SKIP。
  → **09-05 计划的"逐枪显式覆盖表"作废**：一个符号错误就该修一个符号，别建 14 条魔数表去绕它。

  **④ 已做**：改 `detect_axes` 一行 → 重跑 `prep_guns.py`（14/14 ok）→ `install_guns.py` 装 13 跳 1；
  `assets/guns_ext/*.glb`(14) 与 `assets/guns/*.glb`(13) 已全部更新。

  **⑤ ✅ 引擎内实机验证已补上（2026-09-09 同日，推翻本条早先"切枪键没生效"的判断）。**
  之前以为 `Digit2` 没生效——**错了，键一直是好的，我只是猜错了那个槽位里是哪把枪**。日志是铁证：
  ```
  weapons: 切枪 0 -> 1 (AK-104 短剑)
  gun-glb: ak104 ← assets/guns/ak104.glb 顶点=11705 索引=20352 跨度=(0.08,0.30,1.00) align=IDENTITY
  ```
  - 引擎读到的 `跨度=(0.08,0.30,1.00)` 与 `prep_guns_report.json` 里 ak104 的
    `[0.0761, 0.3002, 1.0]` **逐位吻合** → 加载的确实是重生成后的新资产，不是旧文件。
  - `align=IDENTITY` 再次证明 09-05 那条判断成立：引擎侧处理"已规范化资产"本来就没问题，
    错的一直是资产符号。**引擎侧一个字没改。**
  - `screenshots/glbchk_b.png` 放大后逐部件可辨：**准星护圈 + 膛口装置 + 沿顶部延伸的气导箍
    在画面左上、指向场景深处**；弧形弹匣与握把居中；**骨架折叠枪托带肩托板在最近处（右下）**。
    即枪口朝前、枪托抵玩家方向 —— 正确；修前同一把枪是反的。
  → **根因 → 资产 → 引擎内，三级验证闭合。** 13 把走同一条代码路径、同一批重生成资产。
  - 教训（本轮第四次同类）：**"键没生效"的结论来自我只瞄了 HUD 小字、没去读日志。** HUD 那行
    武器名在切枪后仍会短暂显示旧名，我据此判"没切成功"，而 `weapons: 切枪 0 -> 1` 早就写进了
    `logs/<tag>.log.err`。**先看日志再看图。** 且 `cap_safe.ps1` 的游戏日志是 **`.log.err`**
    （stdout 那个 `.log` 常为空文件，本轮我在这儿白绕了一圈）。
  - 另记两条踩过的坑：cmd.exe 传数组要 `-Command "& script.ps1 -Keys @(82,50)"`，写成
    `-File ... -Keys 82,50` 会被并成一个 `8250`；`cap_safe` 结尾 kill 与在飞帧竞争，stderr 会刷一叠
    `device has been lost` ERROR，**那是拆机噪声不是崩溃**（本次 fps 147 正常），别误报成回归。

- 状态：**done**（根因已修 → 14/14 资产重生成 → 逐枪看图 → 引擎内 AK-104 实机确认枪口朝前）

### [2026-09-09] 交接：shop_block 骑楼棚已改（**未视觉验证**）+ 两条新发现

- 日期：2026-09-09
- 发起方：当前会话 AI
- 接收方：下一会话 AI
- 交接类型：迭代结束（含一条**明确未验证**的改动，接手时先看这里）

  **① `shop_block` 骑楼雨棚已改，但没通过视觉验证。**
  原代码（`city.rs::shop_block`）：
  ```rust
  c.deco(Part::new(ObstacleKind::Building, cx, cz + 6.4, 44.0, 2.6, 3.6, 3.95, GRANITE));
  ```
  两层硬伤（**纯坐标算术就能判，不依赖看图**）：
  1. 三排楼 `row_houses(…, d=9.5, 中心 cz)` 的外墙面在 `cz+4.75`，而棚的内沿在 `cz+5.1` ——
     **离它该挂上去的墙还空着 0.35m**，等于飘在墙外；
  2. 44.0 × 2.6 的 footprint 只有 **0.35m 厚**，平着色下顶/底各一块均匀色、边缘无收头 → 剪影就是纸。
  改后：内沿 `cz+4.2`（埋进立面 0.55m）、外沿 `cz+7.9`、厚 0.35→0.6、长 44→41（与楼群外轮廓
  `cx±20.5` 齐平）；另加外缘檐板（3.5→4.42，高出棚顶 0.32）与柱头牛腿；柱顶/牛腿都**伸进棚身
  0.12m 到 3.62**，避免与棚底 3.5 共面闪缝。`cargo test` **458 全绿**（未触发
  `no_degenerate_geometry` / `relief_steps_are_not_coplanar`）。
  - ⚠ **但"改了没变好"没有被任何一张截图证明过。** 原因见 ②③：能对准它的 `RV3D_CAM` 会让 HUD
    谎报 FPS，而画面前景被 ③ 那坨更大的白结构挡住。**下一轮请先修 ②，再重拍验证 ①。**

  **② HUD 大号青色 `FPS 0` 是 `RV3D_CAM` 调试机位分支的谎报，游戏本身没问题。**
  实测：日志整段 `fps=146~152`，`fps=0` 在所有 `logs/*.err` 里出现 **0 次**，无 panic / 无 device lost。
  HUD 上同时有 `VULKAN: 150 FPS`（渲染器自己的计数，正常）和大号青色 `FPS 0`。
  定位：`main.rs` 的 `cam_override` 分支（`RV3D_CAM` 生效时）**提前 `return`**，而喂青色那个字段的
  `pl.frame(self.last_fps, …)`（`main.rs:2032`）在 return **之后**，于是永远不执行、字段停在初值 0。
  对照证据：`spvchk_b.png`（普通玩法，无 RV3D_CAM）青色显示 `FPS 153`；`shopblk_b.png`（带 RV3D_CAM）
  同一二进制显示 `0`。**唯一差别就是那个环境变量。**
  → 这条要修（小改动），否则 `RV3D_CAM` 这个看地图的工具会让人误判"游戏卡死"。

  **③ 中央广场有一坨巨大的白色薄壁交叉结构 —— 来源未查明，且不是 shop_block 的棚。**
  🔴 **本条已于 2026-09-10 结案：那是 `city.rs:1146`「中央隔离带」的混凝土墩 + 压顶，
  见文件顶部 `[2026-09-10]` 交接 ②。下面这段保留为当时的记录，注意它给的筛选阈值是错的
  （按「跨度>15m / 薄轴<0.5m / base>1m」筛，真凶是「跨 6m / 厚 0.9m / base≈0」，被整条过滤掉了）。**
  `spvchk_b.png` / 放大图：两条带侧壁的高架"走廊"在广场中心近直角交叉，底部离地能看见穿过去的缝，
  远处还有一根柱子。`screenshots/spvchk_b.png` 是在**改 ① 之前**截的，所以**它不是本次引入的回归**。
  - ⚠ **我当时没有查出它是谁生成的，这里不猜。** 那天已在"看着像什么就断言"上错过三次
    （枪口根因、复检反噬、把白墙当成骑楼棚）。
  - （当时的）下一轮定位建议：**dump 几何而不是看像素** —— 用 `generate_city()` 把该坐标邻域内的
    `decor`/`obstacles` 连 `w/d/base/top` 一起打出来（临时 `#[test]` 即可），按尺寸筛出
    "水平跨度 >15m、薄轴 <0.5m、base >1m" 的件，就能直接指认生成函数。

- 状态：① done-but-unverified；②③ 已定位/已排除，均**待修**
...
- 交接内容：
  ...
  **⑨ `assets/*.spv` 每次构建都变脏 —— 已查清并修（2026-09-09，同时更正我上一条里写错的结论）。**
  - 🔴🔴 **本条整体作废（2026-09-10 实测推翻）：「已修」是反的，`fcea68e` 不是修好了它、
    而是制造了它。** 该提交把这 7 个文件从「构建真正产出的字节」**换成**了另一次构建的字节
    （`30860 → 33428` 等，**变大** 7%），所以从那以后每次 `cargo build` 都会把它们改回去、
    显示为 modified。逐 opcode 对比两版 `triangle.frag.spv`：**除 `OpName`=112、
    `OpMemberName`=20 外其余 60 多个 opcode 计数逐条相同** —— 差别**只是调试符号名**，
    着色器语义一致（所以两版都能跑）。而 `build.rs:1285`/`:1302` 两处 naga 调用**都传
    `debug_info = None`**，**当前 build.rs 根本生成不出带头名字的 .spv** → HEAD 那份必来自
    另一次开着调试名的构建。下面「实测重跑两次 MD5 相同」之所以通过，是因为两次都在**同一份**
    临时配置下比：同配置内的确定性成立（这条结论本身没错），*跨配置*的差异被漏掉了。
    已于 2026-09-10 提交 `50abf26` 把构建真正产出的那一组入库，见文件顶部 `[2026-09-10]` 交接。
  - 🔴 **我原先写的"naga/rspirv 输出不是逐字节确定"是错的，没测就下了结论。** 实测：强制
    `build.rs` 重跑两次，`triangle.frag.spv` 与 `mesh.spv` 的 MD5 **两次完全相同**
    （`1db5cfa1…` / `8722ee70…`）。生成器是**确定性**的。
  - **真原因是：仓库里提交的 `.spv` 是过期的**——它们与当前 WGSL/工具链编译出来的字节不一致
    （体积差 7%~25%，像是换了 naga 版本后没重生成一起提交）。所以第一次构建必然把它们改成正确
    内容并显示为 modified，**那不是"假"改动，是仓库状态本来就错**。
  - 已做两件事：
    1. `build.rs::write_spv` 加"**内容相同就不写盘**"——实测有效：强制重跑后文件 mtime
       保持 `22:54` 不动。消除无谓写盘与 mtime 抖动（编辑器热重载、任何 watch 这些文件的脚本、
       将来若给它们加 `rerun-if-changed` 连 cargo 自己）。
    2. 把**重新生成的正确 `.spv` 提交入库**，让库里的二进制与库里的 WGSL 对齐。
       ⚠ 注意 1 **不**解决 2：内容真的不同时照样会写、照样该显示 modified，那是应有行为。
  - 提交前已实机验证新二进制渲染正常：`screenshots/spvchk_b.png` —— 沥青裂纹与地面细节层在
    （无黑地）、砖立面真窗洞/烟囱/屋顶细节在、树冠体积正常、混凝土柱有皮肤、AK-104 枪口朝前，
    fps 154。**这 7 个文件是运行时从磁盘读的着色器，不是可随便换的产物。**
  - 以后看到 `assets/*.spv` 变 dirty：先怀疑"库里的是不是过期了"，别急着想成噪声。

- 状态：done（本条）；下一项：**修枪口朝向**
  —— ⚠ 该项已于 **2026-09-09 根治**，且当时记在这里的"根因/逐枪覆盖表/复检反噬"三条判断
  **全部是错的**。**请读文件顶部的 `[2026-09-09]` 交接**（真根因 = `detect_axes` 把两端的
  测量对调，13/13 把枪全反，修法是改那一行），下面 `[2026-09-05]` 里的旧段落仅作为
  错误推理过程的记录保留，已在那条上加了 🔴 作废横幅。
...
- 验收：**`cargo test --release` 457 全绿（mesh 路径下）**；实机截图 `screenshots/mesh1_a.png` / `nosway_a.png`；无 device-lost、无 VUID 报错、`cap_safe.ps1` 确认无残留进程。
  ...
  - **⚠ 枪朝向：缺陷真实存在，根因已定位（2026-09-06），修复待做。**
    - 🔴 **本条以下三段（09-06/09-07 的"根因=消音器骗过 tip-thinness"、"只有 as_val 有硬证据"、
      "修法是逐枪显式覆盖表"）已于 2026-09-09 全部推翻并修复**——真根因是 `detect_axes`
      把两端的测量对调了，**13/13 把枪全部朝后，不是个别枪**，修法是改那一行、不是建覆盖表。
      **请读文件顶部的 `[2026-09-09]` 交接，下面的内容只作为错误推理过程的记录保留。**
    - **✅ 根因已定位（2026-09-06）：误差是 180°（枪口朝向玩家），不是 90°；错在资产不在引擎。**
      - 决定性测量：探针改为输出**世界空间 AABB 各轴跨度**（避免任何轴约定歧义），实测
        `asval 世界AABB跨度 x=0.038 y=0.158 z=0.676 | 相机forward=(-0.00,-0.00,-1.00) right=(1.00,0.00,0.00)`。
        相机看向世界 -Z，枪长轴落在 **z=0.676**、横向仅 **x=0.038**（枪宽 3.8cm）。
        **若真是横躺 90°，这两个数会完全互换。** 故长轴确实平行视线 ⇒ `IDENTITY` 分支与
        `fp_gun_matrix` 变换链**都是对的**；我上一轮"探针测了另一条变换链"的推测也是错的
        （已核对 `fp_gun_pre` 在非检视模式下就是 `self.fp_gun_matrix()`，与绘制同源）。
      - 截图 `screenshots/aabb_a.png` 里**右端（消音器）明显比左端（枪托）大** ⇒ 右端离相机更近
        ⇒ 枪口指向玩家。结合上一条唯一解释是**枪口位于资产的 glTF -Z 端**，即烘焙时符号选反。
      - **根因**：`tools/blender/prep_guns.py` 用"末端更细者为枪口"（tip-thinness）判定，但
        **AS Val / VSS 这类带整体式消音器的枪，前端比枪托更粗**，该启发式必然选反。
        `pkm` / `pkp` 同样走 tip-thinness，需一并复核。
      - **修法（下一轮做，改 Blender 侧，不要改引擎）**：给 `prep_guns.py` 加**逐枪显式覆盖表**，
        对消音器武器直接指定枪口端，启发式仅作默认。候选判据是"最末端 2% 长度内的最小横截面"
        （消音器前端只有小膛口，比枪托端面小得多）——**但须实测验证，不要凭猜替换启发式**。
        重跑 `prep_guns.py` + `install_guns.py` 后必须逐枪截图确认。
      - ⚠⚠ **陷阱说明（2026-09-08 复核代码后更正——原来这段是我写错的，别再按旧版理解）**：
        `detect_axes` 在规范化烘焙之后确实还会被调用一次做独立复检（`prep_guns.py:899`，
        注释明写 "independent check, not an echo of step 4"），但它的**唯一作用**是算出
        `rep["not_canonical"]`（:910）并参与 `rep["ok"]` 的与运算（:942）。
        **它不会施加任何纠正旋转**——我 09-07 写的"触发纠正旋转、正好把刚修好的方向翻回去"
        是**错的**，代码里根本没有这条路径。
        而且 `tools/install_guns.py` **完全不读 `ok` 字段**（只按 `KEY_MAP`/`SKIP` 决定装不装，
        报告里的问题只打印不阻断）。所以只在主判据处加覆盖、不管复检，真实后果是：
        **枪照样会被正确安装和渲染，只是 `prep_guns_report.json` 里那一条 `ok:false` +
        一条 "ORIENTATION NOT CANONICAL" 假告警。**
        → 仍然**应当**让覆盖表同时作用于复检（把复检的 msign 按同一规则翻转后再判规范性），
        但理由是"别让报告说谎"，不是"否则修复失效"。这个区别很重要：我上次就是因为它才
        中止了改动，白白损失一天。
      - 另注：`as_val` 是唯一有硬证据需翻转的；`vss_vintorez` / `pkm` / `pkp` /
        `pp-19-01_vityaz` 外形上前端也可能更粗，但**没有逐枪截图证据前不要批量填**。
      - **方法论（本会话第三次同类坑）**：同一现象我先后误判为「绕序问题」「实例槽越界」
        「读图错误」「90° 偏转」。教训不是"别信眼睛"也不是"别信探针"，而是
        **先设计一个量纲上不可能误读的测量再谈解释**——世界 AABB 各轴跨度就能把 90° 与 180°
        的错误区分开，而我之前两次用的量（投影跨度、截图观感）都能被误读。
    - 状态：**根因已知，修复待做（Blender 侧逐枪覆盖表）**。引擎侧 `IDENTITY` 分支保持不变。
...
- ✅ **LICENSE 已落地**：内容依版权所有者提供的《代码协议.txt》原文写入（AGPL-3.0 + 附加商业条款：开源永久免费 / 闭源季度营收 < 1000 万元自动免费 / ≥ 须书面商业授权）。**未改写法律条款**，只补了一节"第三方素材授权范围"——`assets/guns*` 里下载的枪械带各自来源许可，不能被本仓库的 AGPL 覆盖，引入前须逐个确认可再分发/商用。
  - ✅ 联系邮箱已按作者提供的地址填入：`huangshaojie925@gmail.com`（commit `332b85f`）。
- ✅ **README 已按用户规格重写**（commit `332b85f`）：删除「一句话」介绍与「项目状态」快照表，改为**重大功能上线时间表**；章节顺序严格照用户指定（里程碑 → 当前进度截至 09-01 分已完成/优化待完成/改进待完成 → 配置要求合并为单表 → 硬件门槛与图形 API 要求 → 操作说明 → 联机说明与指导 → Vulkan 特性 + 画面特性 → 项目结构（原文引用既有树）→ 文档索引 → 本地部署 → AGPL-3.0 协议）；英文部分为中文完整对照，同结构同内容未简化。
  - ⚠ 注意：`34e1f1f` 那版 README 因擅自重构表格与章节被用户否决并回退（`1653137`），随后 `332b85f` 才是按用户逐条规格重写的版本。**教训已存记忆：用户说"更新文档"不等于授权重构文档结构。**
- 状态：done（mesh 主路径恢复并验证；道具剔除接线完成；13 把枪模安装；LICENSE 与 README 更新）
- 下一轮顺序：① **修枪口朝向**——根因已知（`prep_guns.py` 的 tip-thinness 启发式对带整体式消音器的枪选反），改法是加**逐枪显式覆盖表**，重跑 `prep_guns.py` + `install_guns.py` 后**逐枪截图确认**；引擎侧不动 ② PT 0xC0000005 专项排查（当前停用）③ 地面场 GPU 侧剔除（mesh 路径剩余帧率天花板）④ 「玩家站进 GLB 楼体」尺寸校准 ⑤ 删 `merge()` 死代码并把它 5 个测试改用超大 cell ⑥ 删 `scripts/allowall.py`/`allowv3.py`/`allowv4.py` 的 `dead_code` 压制规则 ⑦ `svd_63` 人工清理后装为 `svd12`。
