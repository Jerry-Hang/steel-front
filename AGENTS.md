...
17. **`survive` 5 波真机仍未跑通 —— 2026-09-16 实测第 1 波就没清完**（`RV3D_MAP=assets/maps/defense_line.toml`，
    ...
    两处根因，**已修一个**：
    - ✅ **NPC 手榴弹「出手即自爆」**（已修）：从**脚底**出手，而落地判据是 `y <= ground+0.05`，
      出手后第一帧只上升 `vy*dt` ⇒ **≥108fps 时第一帧仍在容差内 → 原地引爆**（8m/120 伤打死自己）。
      现场证据：`grenade: npc #12 throws at (0, 0)` 与 `kill: npc #12 eliminated` **同一秒**，
      而全场 `weapons: shot #` **0 条**（玩家一枪没开却涨了 40 分）。修法 = `NPC_GRENADE_RELEASE_Y`
      （手的高度 1.2m），回归测试 `npc_grenade_does_not_detonate_on_release`（60/130/240fps 三档）。
      ⚠️ **推论：此前所有 ≥108fps 的 NPC 手榴弹局（含压力模式 A/B）里，投掷者都在自杀** ——
      那份 20 轮红蓝对称性结论跑在 130fps，取数前先看这条。
    - ❌ **残余 NPC 卡在 Patrol**（**open**）：自炸掉 4/6 之后，剩下 2 个十几分钟恒为
      `patrol=2 chase=0 attack=0`，不推进 ⇒ 波次永远清不掉 ⇒ 没有波间补给、没有第 2..5 波、没有胜利态。
      **lead**：survive 下 NPC 的目标应恒为玩家/防守点（不依赖视距感知）；属设计决定，**未动**。
