...
17. ✅ **`survive` 5 波真机：2026-09-25 晚首次通关（结案）**（`RV3D_MAP=assets/maps/defense_line.toml`
    ...
    收口 5 次引擎修复 + harness 3 处（换枪判据 / 卡死看门狗 / `hits=` 尺子）的判据与红证见
    `docs/PROGRESS.md` §21 与 §21.6。**剩下的只是枪法**：理想 ≈3.9 发/杀 vs 实际 12 发/杀。
...
22. ~~**`data/` 里的历史残留**~~ **已清理（2026-09-13）**：62 文件 → **只留 3 个被引用的**，screenshots/logs 一并瘦身。
...
42. **🔴 阈值型分支的测试必须取「跨过阈值」的输入，否则等于没测。** `survive` 的 `rule.waves`
   ...
   而线上地图用 5 ⇒ 「清满 3 波就升关并把 wave 归 1」这条真实路径**从没被执行过**
   （第 4/5 波永远到不了、胜利条件永远不成立，2026-09-25 才在真机上抓到）。
