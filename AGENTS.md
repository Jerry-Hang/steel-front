...
**⇒ 但地图侧查不到对应差异**：`tests::dump_half_map_asymmetry` 实测两半场
**障碍 596/585、占地 13441/13422 m²、道具 288/288 ⇒ 掩体占地比 0.999**。
**总量是对称的** ⇒ 差异不在"有多少掩体"，而在**别的位置量**上。

**下一步 lead（收窄后）**：既然掩体总量对称，要量的是**出生点本身**。
`spawn_stress_battle` 的出生环以**玩家（原点）**为心、半径 150m，再经
`push_out_of_obstacle` 推开 —— **+X 与 −X 两侧被推开的位移分布可能不同**，
于是两队实际接敌距离/地形不同。量法：把 `push_out_of_obstacle` 前后的坐标差
按半场统计（**可以在测试里直接算，不必跑游戏**）。

⚠️ **另一条仍未排除**：玩家恒为 `Team::Blue`、无敌、站在**原点** —— 而原点是两半场的分界。
若某个 AI 分支对"离玩家多远"敏感，就会对半场表现出偏好。**但它解释不了"胜负跟着半场走"**
（玩家始终在原点、不跟着半场移动），所以优先级低于上面那条。
