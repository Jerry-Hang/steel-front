...
- 验收：**`cargo test --release` 457 全绿（mesh 路径下）**；实机截图 `screenshots/mesh1_a.png` / `nosway_a.png`；无 device-lost、无 VUID 报错、`cap_safe.ps1` 确认无残留进程。
  ...
  - **⚠ 枪朝向：缺陷真实存在，根因已定位（2026-09-06），修复待做。**
    - **✅ 根因已定位（2026-09-06）：误差是 180°（枪口朝向玩家），不是 90°；错在资产不在引擎。**
      ...
      - ⚠⚠ **动手前必读的陷阱（2026-09-07 读代码时发现，已因此中止一次改动）**：
        `detect_axes` 在**规范化烘焙之后还会被调用一次**做独立复检（`prep_guns.py:912`，注释
        明写 "independent check, not an echo of step 4"），用来确认枪口是否已落在 glTF +Z。
        **如果只在主判据处加 `invert` 而不管复检**，复检会用同一个被消音器骗到的启发式，
        判定"朝向不规范"并触发纠正旋转 —— **正好把刚修好的方向再翻回去**，而且现象看起来
        像"改了没效果"。所以覆盖表必须**同时作用于两处**（或让复检跳过已显式指定的枪）。
        我判断剩余上下文不足以安全完成"改两处 + 重跑 + 逐枪验证 + 处理复检反馈"，
        故**未留下任何半截改动**，`prep_guns.py` 已 `git checkout` 还原，工作树干净。
