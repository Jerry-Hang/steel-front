...
### ✅ 障碍 marker 可见尺寸（2026-09-15 实测 → 2026-09-17 已修 `50b61b9`）

模板半幅的**唯一真源** = `geom::Shape::template_half_extent(axis)`，缩放 = `half/tmpl` 逐轴归一；
🔴 **不要再引入第二处倍率真源**（弹孔侧的 `visual_half_gain` 补偿已删）。
测试 `marker_visible_size_matches_aabb` 锁死（改回 `2*half` 会红）。细节见 `docs/PROGRESS.md` 2026-09-17 §1。
...
1. ✅ **道具焊接流程**（2026-09-19 结案）：**改道具 = 改生成器 → 重跑 → 再焊接**，
   绝不在已焊结果上"补"颜色（24 件已按此重生成，数据与判据见 `docs/PROGRESS.md` 2026-09-19）。
2. ~~**PT 崩溃 `0xC0000005`**~~ **已结案（2026-09-15）：四个真 bug 全修**（交换链缺 `TRANSFER_DST` / `hud_framebuffers` 未随 5 次重建 / `pt_img` 格式与 GLSL 不等 / overlay 复用 MSAA+深度管线）。**判据见铁律 B 的 PT 段**。
...
8. ~~**D4 墙缝天空亮条 / 悬浮亮条**~~ **已结案（2026-09-19，数值巡检）**：檐梁 139–144 < 天空 166；
   楼缝亮条恰=天空梯度上限 166，非缺陷。判据 = `tools/patrol.py` + 行亮度检查（**必须排除小地图列**）。
...
23. **`VUID-VkSwapchainCreateInfoKHR-flags-parameter` 与输入矛盾，按「层侧误报」挂着**（2026-09-15）。
    ...
    **在上述三条被推翻前不要再改代码"修"它。**
24. ~~**广场"坑"**~~ **已结案（2026-09-19）= 水平面绕序反了**（顶面从上方恒被剔除，池沿/水面/花坛/路缘石都是"开口盒"；
    CPU + mesh 两份几何已同步翻转，判据 `horizontal_winding_tests`）。复验走**数值巡检**（图像通道当时只回放旧帧）。
25. **`find_path`（A*）没有节点预算 ⇒ 单帧尖峰**（2026-09-23 实测定位，**未修**）：
    既有日志里 1208 条分项样本显示 `ai_us` **中位 ~0.5ms、最大 41.6ms**，且尖峰与人数**无关**
    （`enemies=1` 就有 41.6ms；`enemies=8` 最大才 1.4ms）⇒ 是"某一只算爆了"。
    候选 = `ai.rs::find_path`：目标不可达会一路展开到整张 128×128 网格（~65k 次堆操作），
