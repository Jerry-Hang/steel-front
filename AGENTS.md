...
7. **D12 士兵远距离读作蓝色平板 / 近距四肢体积感未验证** — 2026-09-12 第 79 轮**更正卡点**：
   **士兵一直在正常渲染**（`soldier_part_matrices` 的 17 段 + 逐队 tint 全部正确），
   此前"看不到士兵"是**被 GLB 建筑遮挡**。取证配方（缺一不可）：
   `RV3D_NPC_CAM=<i>` **且** `RV3D_NPC_POS=1`（只开前者没有 `npc_cam:` 日志），
   读日志里的 `npc=(…)` 与 `相机读数/朝向` 确认目标在视锥内。
   **🔑 二分手段：`RV3D_NO_PROPS=1` 关掉全部 GLB 道具** —— 关掉后出现 = 遮挡；仍不出现 = 渲染问题。
   **⚠️ 不要只用 `RV3D_CAM` 取景**：它让 `update()` 提前返回 ⇒ `on_any_key()` 不执行 ⇒
   **停在开始菜单、世界里没有 NPC**（2026-09-12 拍到过一张开始菜单当证据）。
   **仍未拿到"士兵占画面高度 ≥ 1/4"的图** —— 拿到之前无法判观感。
