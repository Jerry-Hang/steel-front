...
`ab_reps.ps1 -Reps 20`（40 局 × 120s，`blue margin = 红损 − 蓝损`）：baseline 均值 **+12.8**、swapped **−10.3**，
但**臂内极差均值 41.5 > 臂间差 23.1** ⇒ **任何"单次对撞谁赢"都不构成证据**（教训 24/27/35）。
**"红方恒胜"与"−X 半场占优"两个说法都撤回**；四条静态测量全部对称（掩体 / 出生可站立 / 角度 / 半径），
⇒ 要继续先解决"每局方差 ~±30 点"的测不准。细节见 `docs/PROGRESS.md`。

- 相关：军情 JSON 的 `击杀` 字段实际是**该营自身阵亡数**（`round_kills_*` 按阵亡者阵营计数）。
  若 `llm_commander.py` 当"我方战果"读则**信号是反的** —— 调参前先核对。

### ✅ 障碍 marker 的**可见尺寸 = 碰撞盒的 2 倍**（2026-09-15 实测 → **2026-09-17 已修 `50b61b9`**）

`for_obstacle` 三轴一律 `2*half`，而模板是 ±1 的立方体/球、r=1 但 y 已 ±0.5 的圆柱
⇒ 除圆柱高度外全部画成 2 倍；这个"部分正确"的约定让它活了三周
（实测判据：近处圆柱的宽高比 0.61，2× 预测 0.63 ✓、1× 预测 0.31 ✗）。
后果清单与完整修法见 `docs/PROGRESS.md` 2026-09-17 节 §1：`geom::Shape::template_half_extent(axis)`
是模板半幅的**唯一真源**，缩放 = `half/tmpl` 逐轴归一；弹孔侧的 `visual_half_gain` 补偿已删，
🔴 **不要再引入第二处倍率真源**。测试 `marker_visible_size_matches_aabb` /
`marker_model_is_pure_translation_and_scale` 锁死（已实测改回 `2*half` 前者会红）。

0. ~~**`PrintWindow` 对非前台窗口返回冻结帧**~~ **已结案（2026-09-14）：症状不复现**。`cap_safe.ps1` 用 `PrintWindow(h, dc, 2)` = **`PW_RENDERFULLCONTENT`**（别把 2 改成 0）；复测判据 = 同刻两张图差异像素 1.36%（冻结会是 0.00%）。
1. ✅ **24 件道具已全部用修好的焊接重生成**（2026-09-19 结案）。其余 23 件的原色在 `82a2306`
   的焊接结果里已丢失，本轮从生成器重跑：`gen_props.py` 18 件 + `build_city_kit.py` 6 件
   （尺寸契约 6/6 命中）→ 修好的 `weld_props.py`（CORNER 域）。数据证据：24/24 包围盒与在库
   一致、逐件颜色种数回升（container 1→3、建筑模块 19→36）、`tree_oak` 与在库逐字节一致
   （管线确定性的免费证明）；渲染侧 props 顶点总数 534540→515216、fx4→fx5 帧差 2.748%
   集中在天际线。**⇒ 长期规矩：改道具必须改生成器后重跑再焊接，绝不在已焊结果上"补"颜色。**
...
17. **`survive` 5 波真机仍未跑通 —— 2026-09-16 实测第 1 波就没清完**（`RV3D_MAP=assets/maps/defense_line.toml`，
    ...
    - ✅ **NPC 手榴弹「出手即自爆」**（已修，**第二轮复验通过**）：从**脚底**出手，而落地判据是
      ...
      `npc_grenade_does_not_detonate_on_release`（60/130/240fps 三档）。
      **判据（两轮对比）**：修前 4 次投掷 = 4 次**同秒**自杀（`grenade: npc #12 throws` 与
      `kill: npc #12` 同一秒，且全场 `weapons: shot #` = 0）；修后 4 次投掷 **0 次**自杀，
...
23. **`VUID-VkSwapchainCreateInfoKHR-flags-parameter` 与输入矛盾，按「层侧误报」挂着**（2026-09-15）。
    ...
    **在上述三条被推翻前不要再改代码"修"它。**
24. **广场"坑"的真根因 = 水平面绕序反了**（2026-09-19 结案 → 教训 40）。顶面从上方恒被
   剔除，池沿/水面/花坛/路缘石全是"开口盒"，开口露出地面。已修（CPU + mesh 两份几何同步
   翻转），判据 `horizontal_winding_tests`（500 绿）。复验走数值（图像通道仍只回放旧帧）：
   俯视/侧视池心扫描线，修复前恒地面色 (140,132,119)，修复后水面蓝 (101,121,142) 满铺
   8.2m + 两侧 0.4m 灰石沿 ✓。①树冠棕色目视已无；③柱头压顶自下方的可见性随本修复成立。

---

## 教训清单（跨迭代去重合并）

> 40 条，每条都真的付过代价。**只留可执行的判据**，案例细节见 `docs/PROGRESS.md`。
...
28. **视觉改动的验收必须给两个数，且两次运行场景必须一致。** 流程：同场采基线 → 改动 → 重采 → 整幅 diff（第一道筛子）→ 在差异集中区取指标；整幅 diff 的数值**不能当改善幅度**用。**图像通道不可靠时改数值巡检**：代表机位全图扫描 + 逐帧过曝/纯黑/异常色占比，能自动揪"NaN 钳黑"类成像事故（2026-09-19 树冠黑带，判据黑占比 <0.5%）。
...
37. **🔴 截图是「崩溃前的最后一帧」——"改动毫无效果"之前先 grep `has been lost`。** 2026-09-15 查弹孔时依次否掉了追加实例、模型矩阵、颜色、尺寸、遮挡，**而每次 A/B 都在比对两张"设备已 lost、画面不再更新"的旧图**（`pm_play` 按 "2" 切枪触发 device lost，见铁律 D）—— 游戏照常"在跑"、截图照常能存、fps 照常有，**唯独画面是死的**。**⇒ 判据：任何"视觉改动毫无效果"的结论，先在 `logs/<tag>.log.err` 里查 `has been lost` / `panicked`，再去看图。**
...
40. **🔴 "某个面没画出来"先查绕序/背面剔除，再查几何参数**（2026-09-19，代价 = 两天）。本管线
   水平面与竖直面的正面约定相反（CLOCKWISE + shader Y 翻转：水平面从上方可见 ⇔ 与地面 quad
   同绕序），立方体顶/底面与 mesh 圆柱盖长期反绕 ⇒ 顶面恒被上方剔除；平着色 + 法线翻向让
   "透过开口看到的底面"与真顶面逐像素无法分辨，一个绕序 bug 伪装成"建模/透视/烘焙"三类问题。
   **⇒ 两个一次重建的探针足以定性：可疑面片涂成不可能色（顶面→绿）、可疑体积涂红（任何片元→红），
   区分"没进光栅器 / 进了但被盖 / 上了错色"；回归判据 `horizontal_winding_tests`。**
41. **断言"构件没渲染"前先确认相机在它的正面**（2026-09-19）。shop1 从街北拍商铺误报
   "没有雨棚"（骑楼实际在南侧）；cp1 反向实锤：残骸车/帐篷整个
   埋进围合板楼（内缘 ±7.5）只剩墙面悬空黑片 ⇒ `checkpoint_props_stay_out_of_rows`。
