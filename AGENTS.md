...
### ⭐ 最高优先级：红蓝阵营不对称（**已定位到「队伍侧」**，2026-09-13）

压力模式下**红方恒胜**。2026-09-13 做了**互换对照**（`RV3D_SWAP_SIDES=1`
只交换两个半场，不动队伍/人数/角色，是干净的单变量对照；开关在
`game.rs::spawn_stress_battle`，默认关 ⇒ 生产行为不变）：
...
**⇒ 半场对调后蓝方**照样输** ⇒ 赢家跟着**队伍**走，不跟着半场走。**

**⇒ 按原先写好的判据，这**排除了地图几何**，指向**单位/AI 行为**。**
（原先怀疑的「+X/−X 半场 × 程序化城市 × `push_out_of_obstacle`」**可以划掉**。）

**下一步（判据已明确）**：两队唯一的构造差异是
① `per_side` 蓝方少 1（玩家算蓝方，64v63+玩家）；② **玩家恒为 `Team::Blue`、无敌、站在原点**
（`game.rs:2036` 等多处硬编）。**第三个对照臂 = 把玩家阵营改成 Red** ——
若此时**红方开始输**，就锁定是"玩家这个无敌蓝方单位"在扭曲 AI 的索敌/寻路（例如红方被
一个打不死的目标吸住，或蓝方把玩家当集结点从而走进杀伤区）。
**测量工具现成**：`ai:` 日志每秒打一次 `red=<存活> blue=<存活>`，跑 `cap_safe.ps1 -Keys @(82) -AfterKeysSec 125`
再读 `logs/<tag>.log.err` 最后一行即可。
**⚠️ 判据要够长**：125s 时双方伤亡已明显分开（红≈40 / 蓝≈65）；更短会把噪声当信号。
