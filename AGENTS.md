...
7. ~~**D12 士兵近距观感**~~ **已结案（2026-09-13）：士兵改由 `assets/soldier/soldier.glb`
   （1082 顶点/540 三角形）实例化绘制**（`cmd_draw_indexed(…, SOLDIER_INSTANCE_BASE)` 用 `self.pipeline`，
   **不需新建管线**）。🔴 **阵营色 = 队色 × `tint.w = 6.0`**（Authored 标记，不接这条会整身涂成一队色）。
   **仍缺**：骨骼动画（现为整体起伏）、两套队色顶点变体。详见 `docs/HANDOFF-soldier.md`。
8. **D4 墙缝天空亮条 / 悬浮亮条** — **lead**：疑似楼间缝隙的正常天空，需定点复现再定。
9. ~~**mesh 着色器过不了严格 `spirv-val`**~~ **已结案（2026-09-15）**：根因 = naga-30
   `writer.rs` 的 `decorate_struct_member` **无条件**写 `Offset`，而 SPIR-V ≥1.4 禁止对非 Block
   类型写它，mesh 又必须用 1.4。现由 **`build.rs::strip_workgroup_explicit_layout`** 去掉 ——
   🔴 **只去掉 Workgroup 可达类型**（从 `OpVariable`(Workgroup) 出发做类型闭包）；
   带 `Block` 的 `_struct_300/303/308` 动一个字节就是缓冲错位（Workgroup 主机侧永不碰 ⇒ 语义无损）。
   **7 个 `.spv` 全部 exit 0**；两条测试锁住两个方向
   （`mesh_spirv_has_no_workgroup_explicit_layout` / `block_types_keep_their_offsets`，都会红）。
10. ~~**PT 512 盒上限静默截断**~~ **已结案（2026-09-14）：512 → 1024 一次分配 + 一次性告警**。
    代价仅 **0.92 → 1.84 MB**（余量 87%）；⚠️ 超容告警有闩 `Renderer::pt_box_cap_warned`（不刷屏）。
...
12. ~~**`MAX_RIGID_BODIES` vs `MAX_AI`** 溢出静默丢弃~~ **已结案（2026-09-14）——两个常量都已不存在**
    ...
    `set_npc_visuals` / `set_dead_bodies` 超容时**不崩不报**，只是"少了几个兵" —— **已补一次性告警**
    （`Renderer::warn_npc_cap_once`）。
...
14. ~~**道具是否进阴影 pass 未确认**~~ **已结案（2026-09-14）：确实没进，两处都已补**（道具 + 士兵）。
    🔴 **剔除必须用光源视锥**（照抄主 pass 那行的相机视锥 ⇒ 影子随视角缺块）。
    **代价（只切 `RV3D_NO_SHADOW`）：0.5%**。
...
20. **DLSS 立项评估未做**。~~`playtest_perf.py` 未做 Windows 移植~~ **已结案（2026-09-15）—— 它不是没移植，
    是搬不过来**（X11/XImage/`pgrep`/`/proc` 全是 Linux 的），改用 **`scripts/perf_run.ps1`**：
    引擎本就每秒往 `logs/perf_*.log` 写 fps + 各阶段耗时 ⇒ 只需"启动 → 等待 → 读日志 → 统计"，
    **不注入输入、不抓屏**。🔴 **实测噪声底 2.8%（69.7 / 71.8）**，见教训 35。
21. ~~**GLB 加载器忽略 `bufferViews[].byteStride`**~~ **已结案（2026-09-14）：已支持交错布局**
    （修法 = `byteStride.unwrap_or(...)`）。⚠️ 交错缓冲读错时每个数**都是合法浮点数**（不崩不报、
    几何静静变乱麻）⇒ **凡"支持"都要补一条会红的测试**（本条的测试验证过：临时关掉 stride 支持 ⇒ FAILED）。
...
19. **看门狗用「心跳式」，不要按"启动后睡 N 秒"来 arm** —— 早先遗留的看门狗曾把新会话杀掉，伪装成"窗口没出现"。
...
30. **改回源码要用编辑器工具或 `git checkout --`**，不要过 PowerShell 字符串（会留残差）。这是教训 7 的延伸。
    ⚠️ **本 shell 的 `ReadAllText` 按 GBK 解码**（65918 B 的文件只读出 38703 字符）⇒ 针对中文的替换**全部静默打不中**，
    而按"读到的行号"删除会**删掉别处的行**（AGENTS.md 被改坏，靠 `git checkout --` 恢复）。
    **⇒ 中文文档的编辑一律走编辑工具**；非要用 shell，先验 `(ReadAllText).Length -eq (Get-Item).Length`。
31. **🔴 遇到视觉缺陷，`rg` 代码注释应当是第一动作。** 本仓的处理方式是「改掉 + 在注释里留一段事后分析」，所以用**现象的词**搜注释常常直接命中同一个缺陷的历史。曾为"薄壁"连猜五个假设，而它就在注释里写着「白色交叉薄壁」。
...
34. **🔴 参数语义要读注释，不要靠"同一套网格"外推。** 答案一直写在表头注释里、我只读了一半。**另一面：那三轮的实机截图看起来都变好了 —— 但改善来自别的因素。⇒ 观感改善只能证明"改动有效果"，不能证明"数值变对了"。**
...
36. **🔴 「工具跑不起来」本身就是一条要修的缺陷，不是环境噪声。** 验证层因为 mesh.spv 的布局被拒而**灰屏**，
    ...
    修掉根因当天第一次开起来，**立刻**报出两条一直存在的 VUID。
