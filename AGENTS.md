...
> **本文件的体量约定（2026-09-12 加入，请遵守）**
>
> 本文件每次会话都会被完整注入上下文，属于**稀缺资源**。它曾经膨胀到 200KB，其中
> 三分之二是重复段落、被推翻后仍留在原地的旧结论、以及 WSL2 时代的失效内容——甚至
> 出现过"同一条铁律的错误版与更正版并存"，和"用户明确给的正确做法被后一句相反
> 断言覆盖"（见下方【教训清单】第 1、2 条）。
>
> 因此：**只写仍然生效的约束与注意事项**——铁律、未结案清单、教训清单、验收红线。
> 被推翻的结论**直接删掉**，不要保留"错误 + 更正"两段；已结案的排查过程只留一行结论。
> **带时间线的进度、状态、迭代记录一律写进 `docs/PROGRESS.md`，不要写回本文件。**
> 目标体量 **< 48KB**；**硬上限 65,536 字节**（超过会被静默截断，那比超标危险得多）。
> 加内容前先想"这条三个月后还成立吗、别人会不会照它做错事"。
...
> 手动流程 = 启动后点一下游戏窗口 → 按 R 进入操控；死亡后再按一次 R 复活。`RV3D_AUTOSTART=1` 跳过菜单。
...
- 🔴 **`RV3D_AUTOSTART=1` 不跳过开始菜单**（2026-09-12 实测）：它拍出来的是
  "菜单 + 背后渲染的世界"。要进游戏态请用 `RV3D_NPC_CAM`（它自己会调 `on_any_key()`）。
  本文件旧版写它能跳过菜单，**与实际不符**。
...
7. **D12 士兵近距观感（用户说"神人样子"）** — 2026-09-12 已修三处、**根因已定案**：
   - **根因（第 55~79 轮查错方向的原因）**：`npc_occluded()` 以**玩家眼位**为中心剔除
     ⇒ 调试相机移动后玩家仍在原点 ⇒ **相机眼前的士兵被全部剔掉**（实测 `npc=288` vs `4590`）。
     **取证必须带 `RV3D_NO_NPC_CULL=1`**（详见铁律 C）。
   - **已修（`renderer.rs::soldier_part_matrices`）**：① 枪悬在身前而手臂垂在身侧、两者永不相交
     ⇒ 改为**持枪姿态**（kind 5~8 用 `HOLD_*` 定角，不再随步伐摆）；② 17 段共用一个 tint
     ⇒ 整个人是一块均匀色 ⇒ 改**逐段明暗**（盔 0.55 / 背心 1.00 / 靴 0.42 / 枪 0.30 …）；
     ③ 背心(1.23~1.53)与头(1.40~1.64)**重叠 0.13m**（头有 43% 埋在背心里）⇒ 按真人重排为
     背心 1.20~1.50 / 头 1.50~1.74 / 盔 1.645~1.795；④ 加背包段（盒 9→10）。
   - **仍未解决**：**它的本质仍是"箱体与圆柱的堆叠"**，近看读作机械构造而非人。
     **再调数字收效有限 —— 需要一次真正的建模**（Blender 里做带烘焙 AO 的士兵 GLB，
     按铁律 D 的设计化链路）。取证基线见 `screenshots/soldier_zoom6.png`（第 79 轮前后对照）。
...
26. **安全网的假警报和漏报一样有害。** `release_input.ps1` 在进程刚被 kill、
   窗口尚未销毁时单次判定，会误报 `RELEASE FAILED`——而**同一条命令紧接着再跑就是 OK**。
    ...
    不能只查一次。**
27. **🔴 先确认你的测量工具测的是你以为的东西。** 2026-09-12 一个会话里为此栽了**四次**：
    ① `cap_safe -Keys` 收的是 **VK 码**，我按 winit 序号发 → 游戏零响应；
    ② 截图"待测区域"里**含 HUD 小地图**，它贡献的亮像素一直主导指标；
    ③ 均值型指标只留 1 位小数，**1.2% 的像素变化被四舍五入吃掉** → 误判"改动无效"；
    ④ 全图 diff 里**含小地图与 NPC**，两次运行场景不同 → 差异不全是改动造成的。
