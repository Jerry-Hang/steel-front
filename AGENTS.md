...
- 🔴 **核对"文档里的常量值 vs 源码"时，别写正则**（2026-09-14 一晚第四次假信号）：
  我用一个带字符类的正则去核对 `PT_MAX_BOXES` / `MAX_SOLDIER_INSTANCES` / `PLAYER_STEP_UP`，
  **三个全报"源码里没有"，而三个全都是对的** —— 因为真实写法是
  `pub const PT_MAX_BOXES: usize = 1024;`，中间夹着类型标注和一个额外的 `=`，
  我的模式到不了那个数字。
  **⇒ 判据：核对定义就用 `rg --fixed-strings "const $NAME"` 把整行打出来用眼睛看。**
  这一类"我的匹配模式比数据复杂"的坑今晚一共踩了四次
  （符号名频次 / 字面路径 vs 基名 / 上述正则 / 行号跨版本比对），
  **四次同一形态：先有结论，再写一个刚好能"证明"它的工具。**
...
1. ~~**`PrintWindow` 对非前台窗口返回冻结帧**~~ **已结案（2026-09-14）：症状不复现**。
    2026-09-12 的记录是"注入 170.3° 转向后前后两张截图残差 0.03、x-shift=0"。
    **2026-09-14 复测**：同一次运行抓两张（同刻 `_a`/`_b`）实测"有差异像素 **1.36%**、平均差 1.39" ——
    **冻结帧的差异会是 0.00%**，非零即说明画面是活的（那点差异是 NPC 走动）。
    而且**整晚几十张截图（士兵/阴影/不同机位）都抓到了正确内容**，没有一张是冻结的。
    **⇒ 原因**：`scripts/cap_safe.ps1` 用的是 `PrintWindow(h, dc, **2**)` ——
    flag 2 = **`PW_RENDERFULLCONTENT`**，它正是让 DirectX/Vulkan 窗口能被抓到内容的标志。
    **⇒ 那个 flag 应当是 2026-09-12 之后加的，本条因此过期。**
...
3. ~~**`config.rs` 不读 `pt_enable` / `rt_enable`**~~ **已结案（2026-09-14）——接线早已完成**：
    ...
    **⇒ "配置文件与环境变量都开不了 PT"这个描述与现状相反。**
    ...
    "关=可玩" ⇒ 开起来仍然崩）。
4. ~~**玩家可能站在 GLB 楼体内部**~~ **已结案（2026-09-13）**：根因是 `pick_building` 用
    `scale = max(w/gw, d/gd)` ⇒ 另一方向必然溢出 footprint（52 处 `building_tall` 互相穿插）。
    现改为 **`min`** + 碰撞盒取**真实视觉尺寸**（`half_footprint × scale`，±90° 时 x/z 互换）。
...
9. **mesh 着色器过不了严格 `spirv-val`**（根因已定位，2026-09-14 本轮放弃修）。
    ...
    根因在 **naga-30.0.0 `src/back/spv/writer.rs:3597`**：`decorate_struct_member` **无条件**写
    `Offset`，不区分存储类；网格输出被它落成 `Workgroup` 变量 `%_struct_21`。
    `Offset` 在 SPIR-V ≤1.3 允许、**1.4 起对非 Block 类型禁止**，而 mesh 因 `MeshShadingEXT`
    必须用 1.4。已排除 `global_needs_wrapper`（对 Workgroup 直接 `return false`），
    naga 的 `WriterFlags` 也**没有"别写 Offset"的开关**。
    **下一个人从这里开始**：剥 `Offset` 语义上安全（WGSL 无布局控制，naga 只是抄了自己算的
    MemoryLayout），但**只能剥 Workgroup 可达类型** —— `_struct_300/303/308` 带 `Block`、
    `_struct_9/10/24` 是 Uniform/StorageBuffer，动了就是缓冲布局错位。
    🔴 **别用 PowerShell 打印 cargo warning 来调试 build.rs**：warning 会被归并/缓存，同一个 exe
...
19. **呈现层欠账**：毛玻璃菜单非真模糊（半透明暗色遮罩近似，需 shader 后处理采样主 pass）；
    ...
    缺口其实在**结算层**：`damage_npc(idx, dmg)` 不带来源，feed 想写"你击杀了…"也没信息。
...
24. **单次 A/B 说明不了任何事：必须做「互换对照 + 无处理对照」。** 曾据此得出会反转的结论。**判据：先确认对照组本身没有一边倒，再去看处理组之间的差。** ⚠️ **2026-09-13 又踩一次**：见"红蓝阵营不对称"那条 —— 臂内方差 40 点、臂间差 8 点，我却先写了结论。**量化版（2026-09-15）**：`perf_run.ps1` 连测两次同一二进制，中位 fps 69.7 / 71.8（**差 2.8%**）⇒ **< ~5% 的帧率差必须多轮重复才能开口**。
