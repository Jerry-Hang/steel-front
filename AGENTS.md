...
- 交接内容：
  ...
  **⑤ ⚠ 未完成、下一轮必须补：引擎内 GLB 枪的实机截图。**
  资产侧已逐枪对图验证，但**游戏画面里还没看到过任何一把 GLB 枪以修正后的朝向渲染**。
  卡点：`cap_safe.ps1 -Keys @(82,50)` 的 `Digit2`→`switch_weapon(1)` 没生效（HUD 始终 `AK-12M 风暴`，
  中途闪过 `SWITCHING…` 又回到 AK-12M）。而且 **R(82) 是否真生效也没被证明**——游戏可能在无点击时
  本就进 Playing，所以"PostMessage 按键可用"这个前提本身可疑。
  下一步：读 `logs/*.log` 里 `weapons: 切枪` 那行 INFO/WARN，确认槽位 1 是什么、有没有越界回退；
  或给 harness 加一个"启动即装备指定枪"的环境变量，**别再猜键**。
  顺带：`cap_safe.ps1` 结尾 kill 与在飞帧竞争，stderr 会刷一叠 `device has been lost` ERROR，
  **那是拆机噪声不是崩溃**（本次 fps 150 正常），别误报成回归。
  另注：cmd.exe 传数组要用 `-Command "& script.ps1 -Keys @(82,50)"`；写成 `-File ... -Keys 82,50`
  会被并成一个 `8250`（本轮踩过）。

- 状态：done（根因已修、资产已重生成并逐枪看图确认）；**实机 GLB 验证 = 待做（见 ⑤）**
