...
> 目标 **< 48KB**；**硬上限 65,536 B** —— 实测**超出一点点就真会被截断**（2026-09-26 末条教训被截掉，
> 文件本身没坏、注入进来的是残的）⇒ 改完必看字节数、留 **≥1KB** 余量。加之前先问："三个月后还成立吗"。
...
- 验证层 `RV3D_VALIDATION=1`（默认关）—— 曾因 mesh.spv 过不了严格 spirv-val 而**灰屏**（#9），
  🔴 **2026-09-15 修掉根因后才真能跑**，是**本仓最强的排障工具**（一开就抓出两条 VUID）。
...
> （实测 `SendInput` 喂前台队列 6/6 被接受、游戏零收到；`PostMessage` 才进目标窗口队列。）
...
- **顶点预算**：`props: 缓冲扩容 顶点 N/2097152` ⇒ **硬容量 2^21 = 2,097,152**，加细节前先看这个数。
  ...
  **顶点还是唯一的杠杆**（道具开销正比于顶点数）⇒ **资产改动先用 `propdraw:` 的几何量验收，
  ...
  ⚠️ **别再引用"再砍 17% ≈ +3%"**（那是道具占 ≈27% 帧时间时的账）：2026-09-26 控制臂实测**全部道具
  只占 ≈5.4% 帧时间**（§21.52）⇒ 同比例的顶点削减只值 ≈+0.9%。
- 🔴🔴 **道具/GLB 的两条通则（改动前先读）**：
...
45. **🔴 成本地图（任何"关掉某个东西"的对照实验）必须放「物理上必然更快」的正对照臂，
  ...
  `NO_MARKERS` −5.6% —— 物理上不可能（§21.27 同臂是 **+37.7%**）；机理：外部干扰周期（分钟级）
  **远长于**交替周期（25 秒）。② 改成"每臂 2 对、按臂串行"再撞一次：
