...
24. **单次 A/B 说明不了任何事**：必须「互换对照 + 无处理对照」，先确认对照组本身没有一边倒。⚠️ **小于 A/A 噪声底的差不算数**（教训 43/45）。
...
45. **🔴 成本地图（"关掉某个东西"的对照实验）三件套：正对照臂 + 同轮轮转 + 每臂 ≥5 对。**
  ...
  **远长于**交替周期（25 秒）；明细见 §21.27/§21.45/§21.61。
  ③ 🔴 **每臂只有 1 对时，噪声本身就有 ±20%**：同日 A/A 实测（`ab_pair -Pairs 4 -Secs 25`
  且 `-ExeA == -ExeB`）**中位 −5.51%**、单对 −0.7/−0.8/−23.6/−10.2%、一臂极差 24.1%
  ⇒ **1~5% 的效应在这个窗口里根本测不出来**。
