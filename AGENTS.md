...
- 本轮方法：每条都走「查代码 → 断言/截图取证 → 改 → 同机位复拍」。中途**三次推翻自己的判断**（见 ①⑤⑦），都记在这里。
  ...
  **① ✅ 结案 —— 靠的是一张**正侧对**对照图，不是第 11 次读那张裁剪。**
  - **决定性对照 = `median_side_b.png`**（`RV3D_CAM=fly:9,1.2,-14:90,0`，正侧对隔离带）。
    侧视时**高度与进深不互相冒充**，画面里隔离带是**一道落地的实心灰墙 + 顶上一条深色压顶**，
    没有任何"悬空/开口/看穿"的迹象。
  - **同一物体、同一帧引擎，端视（`#250bfec9fa3a`）却读成"一只浮在路上的浅盘"** ⇒
    那是**端视透视**造成的：相机 y=1.7 **高于**护栏顶 y=1.2，沿长轴俯视时
    ① 墩身侧面的**远段**投影到压顶顶面**近段**的上方（看着像"墙高出板"）；
    ② 压顶比墩身宽且长（1.2>0.9、6.3>6.0）→ 板边**悬出**墙外、近端下方露出**护栏前面**的路
    （看着像"板悬空、路从底下过"）；③ 压顶顶面铺的是与路面同一套 procedural 方砖皮肤
    （设计如此）→ "砖缝与路面完全连续"**不是**看穿的证据。
  - **⇒ 不存在"盒子缺 +Y 顶面"这个 bug。** 本轮我为它翻过 **10 次**、
    两次"结案"靠的都是无效对照（低于墙顶的机位、GLB 道具的楼顶）。**别为 ① 动 shader。**
  - **仍然成立的真错误（已修 `d51afab`）**：压顶做成**比墩身宽 0.30m、长 0.30m**，
    而真泽西护栏是底宽顶窄 —— 正是上面 ② 那条"板悬出墙外"的来源。改成三段收分
    （下 1.00 / 上 0.62 / 顶带 0.82，总高 1.20 与反光柱不变）后不再有部件悬在底座之外。
  - **① 剩下的只是配色观感（可选，只动 `city.rs`，不碰管线）**：出生点这条视线上，
    近白护栏 + 大面积受光铺装 + 浅色柱廊/街墙 三层浅色叠在一起，平着色无渐变无深度线索
    → 整片偏"白纸"。要压一档：压暗这几类 tint、或让它们换 `Shape` 不走方砖皮肤、
    或按 09-10 选项把中央那 4 组隔离带挪远/降矮。
  - **⚠ 本轮最贵的方法论事故（写死）**：我在**同一张斜透视局部裁剪上翻了 10 次**，
    每次都用没验证的输入（hFOV、距离、`meshgen::box()` 的归属、"同纹理⇒同表面"这个错前提）。
    **真正一次定案的是换一张能解耦高度与进深的机位（正侧对）拍一张。**
    **规矩：① 判"某个面没画/某物体形状不对"，先拍**正侧对或正俯视**，不要用斜透视裁剪推理；
    ② 拿图当对照前，先确认那个机位**看得见**被对照的那个面；
    ③ 取证图必须带 provenance（文件名 + 哪个 commit 的 exe + 相机高度）；
    ④ 反算尺寸前，FOV 与距离从代码/坐标查，不能估。**
