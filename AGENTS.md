...
23. **`VUID-VkSwapchainCreateInfoKHR-flags-parameter` 与输入矛盾，按「层侧误报」挂着**（2026-09-15）。
    ...
    **在上述三条被推翻前不要再改代码"修"它。**
24. **广场三件改动已提交、未获实机复验**（2026-09-17 收工）。`28395e8`（删花坛土面盒）、
   `69e1aff`（封冠下裙 + 实心水池）、柱廊方压顶都已提交，但**图像通道从 `zz21_b` 起只回放同三张旧图**
   ⇒ 目视复验拿不到新帧。已改用数值判据结案：`scripts/png_diff.py` 把 `finalA_b` 与各历史帧比，
   差异比随构建新旧**单调**（vs `v12_planter_b` 83.2% → vs `v18_planter_b` 16.7%）⇒ 提交确实进了渲染。
   **⇒ 判据：怀疑"改了没生效"时，先用差异比对构建时间轴定位是哪一批帧失效，别去调几何。**
   🔴 **仍待复验**（下轮第一件事）：①冠心还有无棕色 ②水池是否"一道石边 + 一层漫出的水"
   ③柱头是否一个有侧立面的方块。机位 `RV3D_CAM=fly:0,1.7,2:0,4` + `RV3D_NO_NPC_CULL=1` + `-Keys @(82)`
   （**别用 1.3m 近距**，那会自己造出"坑"的错觉）。若①仍为是：**不要再用眼睛调裙团半径** ——
   先把 `target/seal_test.py` 的射线改成从相机位置向冠心发**带俯仰角的锥**，让测试先红、再据此定参数
   （判据必须先于修改）。
...
> 39 条，每条都真的付过代价。**只留可执行的判据**，案例细节见 `docs/PROGRESS.md`。
