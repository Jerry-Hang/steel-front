...
7. **D12 士兵近距观感（用户说"神人样子"）** — 2026-09-12 **根因已定案**（见铁律 C 的
   `RV3D_NO_NPC_CULL` 那条）：第 55~79 轮查错方向，是因为**相机眼前的士兵被玩家中心剔除掉了**，不是模型、不是遮挡、不是取景。
   **已修**（`renderer.rs::soldier_part_matrices`）：① 枪悬在身前而手臂垂在身侧 ⇒ 改为**持枪姿态**
   （kind 5~8 用 `HOLD_*` 定角）；② 17 段共用一个 tint ⇒ 改**逐段明暗**（盔 0.55 / 背心 1.00 /
   靴 0.42 / 枪 0.30）；③ 背心与头**重叠 0.13m**（头 43% 埋在背心里）⇒ 重排为背心 1.20~1.50 /
   头 1.50~1.74 / 盔 1.645~1.795；④ 加背包段（盒 9→10，预算 83%）。
   **仍未解决**：本质仍是**箱体与圆柱的堆叠**，近看读作机械构造而非人 ⇒
   **再调数字收效有限，需要一次真正的建模**（Blender 做带烘焙 AO 的士兵 GLB，按铁律 D 的链路）。
