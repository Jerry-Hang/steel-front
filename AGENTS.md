...
17. **`survive` 5 波真机：第 1 波已清（2026-09-25），2..5 波未通**（`RV3D_MAP=assets/maps/defense_line.toml`
    是这张图**唯一**的开启方式；`waves = 5` 在 TOML 里）。驱动 = `scripts/run_survive_pm.ps1` + `survive_pm.py`。
    已修并复验：① NPC 手榴弹「出手即自爆」（`NPC_GRENADE_RELEASE_Y`，见教训 38）；
    ② 「目标已知」与「敌人可见」拆两条通道（`target_known`，`90605b1`）；
    ③ **2026-09-25 四条根因链**（包抄目标点跳变 / 寻路起点在墙里 ⇒ 100% 失败 / 导航网格把玩家封成孤岛 /
    覆盖率规则引入的凹角磨死）—— 每条的判据、红证、数字见 `docs/PROGRESS.md` §21 与铁律 H。
    核显实测（`RV3D_GPU=igpu -Secs 300`）：**`waves cleared: ['1']`**、波间补给触发、进入 wave 2 打到剩 3 只、
    `kills/shots 9/124`、`VUID=0 panics=0 device_lost=0`；冒烟（程序化城市）`ALL-OK`。
    **剩下的瓶颈在 harness 枪法**：① 相机 yaw 会累积到 −600° 以上而 `aim` 仍判「已收敛」；
    ② 残局 NPC 反复进出 `Attack`、`stand` 行每几秒刷新 ⇒ 瞄旧行打空、且行刷新会重置它的尝试预算。
    口径：`RV3D_INVINCIBLE=1`（否则先死）；**失败分支（玩家阵亡）仍只有单测覆盖**。
    ~~手榴弹弹道落点受出生点影响~~ / ~~AoE 不结算障碍~~ / ~~切枪无动画~~ 均已结案（2026-09-15）。
...
25. **`find_path`（A*）单帧尖峰：`ai_us` 最大 41.6ms（2026-09-23 定位，**未修，且原因未证实**）**：
    既有日志 1208 条分项样本：中位 ~0.5ms、最大 41.6ms，尖峰与人数**无关**（`enemies=1` 就有 41.6ms）
    ⇒ 是"某一只算爆了"。⚠️ **2026-09-25 更正**：旧记录把它归因于"目标不可达 ⇒ 展开整张 128×128 网格"，
    但真机 `aidiag: astar 1s` 显示 **`连通域穷尽` 恒为 0**（`起点阻挡` 才是当时的 100%）⇒ 该归因不成立，
    **尖峰来源仍未查清**。已顺手降掉的是"每次调用分配三份 O(格数) 缓冲"里的浪费（未做）。
    **lead**：① 给 `find_path` 加节点预算 + 失败短时缓存；② scratch 复用 + generation 戳（免清零）；
    ③ 先加**分项计时**（展开节点数 / 堆操作数）再谈优化。
    **判据**：改前后各跑 `perf_run.ps1 -Secs 30` 看 `ai_us` 的 p95 与最大值（中位本来就不高），
    并用 `RV3D_AI_DIAG=1` 的 `aidiag: astar 1s 内 calls=… fails=…（起点阻挡/目标阻挡/连通域穷尽）` 分项核对。
...
37. **🔴 截图是「崩溃前的最后一帧」——"改动毫无效果"之前先 grep `has been lost`。** 2026-09-15 查弹孔时
   依次否掉了追加实例、模型矩阵、颜色、尺寸、遮挡，**而每次 A/B 都在比对两张"设备已 lost、画面不再更新"
   的旧图**（`pm_play` 按 "2" 切枪触发 device lost，见铁律 D）：游戏照常在跑、截图照常能存、fps 照常有，
   **唯独画面是死的**。**⇒ 判据：任何"视觉改动毫无效果"的结论，先查 `logs/<tag>.log.err` 里的
   `has been lost` / `panicked`，再去看图。**
