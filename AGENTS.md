...
- 本轮方法：每条都走「查代码 → 断言/截图取证 → 改 → 同机位复拍」。中途**三次推翻自己的判断**（见 ①⑤⑦），都记在这里。
  ...
  **① ⚠ 隔离带"纸片化"：几何是对的，我一度判成渲染 bug，收回来。**
  - **改前现象**（`green_spawn2_b.png` 原生分辨率放大）：一道亮平板横在路中间，两侧各一道矮墙，
    板下方还能看到路面 → 读作"一只没有盖的浅盘浮在沥青上"。
  - **我中途的错误结论（记下来别再犯）**：我据此判"压顶的顶面没上屏、视线穿进去看到了下层顶面"。
    **原图否证了它**：亮板的左右边缘**就是**整个轮廓的最外沿，两道"墙"在它的**内侧**，
    板下方露出的路面是护栏**近端面之外**的地面。这三条**全是实心盒子被俯视 + 掠射角的正常投影**
    ——侧墙的远段本来就会投影到顶面近段的上方。图里**没有任何一个特征是一个实心盒子做不到的**，
    所以不需要"少画一个面"这个假设。**下一轮别再为 A 去动 shader。**
  - **真正的原因（候选 B）**：压顶顶面是**近白色 + 铺着和路面同一套方砖皮肤**的大水平受光面，
    在定向光下被打成全画面最亮的一块 → 读作"从路面上揭起来的一张地砖"。
    这才是"纸片化"，**是配色/贴图分配问题，不是几何问题，也不是绕序问题。**
  - **本轮已改的部分（`d51afab`）**：剖面从"压顶比墩身宽 0.30m"（真泽西护栏是底宽顶窄，写反了）
    改成三段收分，顶带 1.2→0.82m、受光水平面 7.6→4.9m²、多出两级台阶轮廓。
    **这条对 B 正是对症的**，改后 `median_taper_b.png` 已读作实体矮墙。
  - **还欠的一步（下一轮，风险低、收益直接）**：把顶面/压顶的**配色与皮肤**从"和路面同源"改掉——
    要么换 `Shape`（不走方砖那套 procedural 皮肤），要么把 CONCRETE_DARK 再压暗。
    这条不需要动 shader，只动 `city.rs` 的 tint/shape 选择。
  - ℹ 顺带排除：09-09"未视觉验证"的 `shop_block` 骑楼棚、`spvchk_b.png`"白色交叉薄壁"、
    以及一直没归因的"悬浮走廊"，**不要再挂在"顶面没上屏"这条已否证的假设上**。
