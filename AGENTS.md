...
### 红蓝阵营不对称：**是位置效应（−X 半场占优）**，成因仍未定位（2026-09-14）

**已确认的**：把两个出生半场对调，胜负跟着翻转 ⇒ **赢家由"谁在 −X 半场"决定，
与队伍、AI、人数、角色都无关。**

**8 轮 × 2 臂（`scripts/ab_reps.ps1 -Reps 8`）：**

| 臂 | n | 均值 | 最小 | 最大 | 极差 | 形态 |
|---|---|---|---|---|---|---|
| baseline（红 +X / 蓝 −X） | 8 | **+16.5** | 6 | 23 | 17 | **8/8 全正** |
| swapped（红 −X / 蓝 +X） | 8 | **−6.3** | −22 | 6 | 28 | 7/8 为负 |

臂内极差均值 22.5，臂间差 22.8 ⇒ **效应勉强超过噪声**。
`blue margin = 红损 − 蓝损`（正数 = 蓝方占优）。

**⇒ 静态地图里查不到对应差异 —— 两条测量都是对称的：**

| 测量（测试） | 结果 |
|---|---|
| `dump_half_map_asymmetry` | 障碍 596/585、占地 13441/13422 m²、道具 288/288 ⇒ **掩体占地比 0.999** |
| `dump_spawn_ring_blocking` | +X 被挡 **39.1%** / 平均推开 2.62m；−X **39.4%** / 2.46m ⇒ **比值 0.992** |

**⇒ 结论：成因**不在静态地图几何**。这一整类解释（掩体分布、出生点可站立性）已排除。**

**下一步 lead（再收窄）**：既然静态几何对称、而效应又确实跟着半场走，剩下的候选是
**运行时/动态**的量。按性价比排序：
1. **玩家在原点**：恒为 `Team::Blue`、无敌。若某个 AI 分支对"到玩家的距离/朝向"敏感
   （索敌权重、集结点、逃跑方向），它会**对半场表现出偏好** —— 因为玩家虽然不动，
   但两队的出生半场相对它是镜像的，而任何**非镜像**的响应（例如"优先朝 +X 推进"、
   或"撤退方向固定"）都会打破对称。**验证法**：把玩家挪到 `(+40, 0)`，看效应是否移动。
2. **NPC 创建顺序**：红方先建（`side == 0`）⇒ `next_npc_id` 更低。若 AI 在同一帧内按
   ID 序结算且存在顺序相关（先手索敌/先手开火），就会产生**与半场无关**的偏置 ——
   这解释不了"跟着半场走"，但值得一并排除。
3. **帧内迭代顺序 vs 出生位置的耦合**：`self.npcs` 的顺序 = 先红后蓝、各自按 `i` 递增，
   而 `i` 同时决定 `angle = base_angle + spread` ⇒ **数组下标与空间方位是耦合的**。
   若任何遍历有截断（`take(n)`、上限、降频 `AI_FAR_DECIMATE`），被截掉的会**始终是同一侧**。

**⚠️ 第 3 条最可疑**：`AI_FAR_DECIMATE` 按"距离玩家的远近"降频，而两队相对玩家的
几何是镜像的 —— 镜像本身不产生偏置，但**只要有一处用了绝对坐标的奇偶/符号**
（例如 `x as u32 % 2`、`x > 0.0` 分支），偏置就会出现。**`rg` 一遍 `x >` / `x <` / `% 2`
在 `ai.rs` 里的用法，比再跑实验便宜得多。**

🔴 **这个结论我先后写错过两次**：第一次据**单次** A/B 写下"赢家跟着队伍走"并入了库，
同一条命令重跑就反号（+51 / +11）；第二次据 4 轮/臂的"15.8 vs 14.0"写下"是地图几何"，
而**地图测量显示两半场 0.999 对称** ⇒ 那条也被我自己推翻。
**⇒ 8 轮/臂才看清方向；4 轮只能看出"可能有"**（教训 24 / 27）。
