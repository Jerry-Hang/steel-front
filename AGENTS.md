...
> 注入进来的是残的）⇒ 改完必看字节数、留 **≥1KB** 余量。加之前先问："三个月后还成立吗"。
...
- 验证层 `RV3D_VALIDATION=1`（默认关）—— 曾因 mesh.spv 过不了严格 spirv-val 而**灰屏**（#9），
  ...
  **改 pipeline / swapchain / 同步 / 描述符前先开它跑一轮**；交换链/重建路径用
  ...
  VUID=0、fps ≈164.7）。🔴 判据须读 **`.log.err`**（引擎只写 stderr，stdout 恒 0 字节 —— 以前它读的是空
  文件，见 §21.51）。
...
- 🧊 **HUD 磨砂玻璃**：**只有「本帧 HUD 里有 `Quad::glass`」时**才把 HUD 挪到
...
45. **🔴 成本地图（"关掉某个东西"的对照实验）三件套：正对照臂 + 同轮轮转 + 每臂 ≥5 对。**
  ...
  **远长于**交替周期（25 秒）；明细见 §21.27/§21.45/§21.61。
  ...
  单对低到 −23.6%、一臂极差 24.1% ⇒ **1~5% 的效应测不出来**。
