...
> 的教训条目，文件本身没坏、但注入进来的是残的）⇒ 改完必须看字节数，留 ≥1KB 余量（注入预算随其它会话内容浮动）。
...
**21 世纪架空世界观的大战场 FPS**（⚠ 旧版曾误写为「二战题材」，据此建模会全错；
...
- 阴影其它约定：UV 必须 V 镜像 `uv.y = 1-(p.y*0.5+0.5)`；光方向语义 = **表面→光源**
  ...
  🔴 两张图**必须在 init 时先转 `SHADER_READ_ONLY_OPTIMAL`**（否则采样到未转布局的那张 =
  VUID-08114），且**主 descriptor pool 的 SAMPLED_IMAGE 计数同步 +1**。
...
- 验证层 `RV3D_VALIDATION=1`（默认关）—— 以前它因 mesh.spv 过不了严格 spirv-val 而**灰屏**（#9 的副作用），
  🔴 **2026-09-15 修掉根因后它才真的能跑**，是**本仓最强的排障工具**（一开就抓出两条 VUID）。
  **改 pipeline / swapchain / 同步 / 描述符前先开它跑一轮**；交换链/重建路径用
  ...
  `run_survive_pm.ps1 -Secs 400 -NoShot` ⇒ **318s 通关**（1..5 全 cleared、52 杀/640 发、VUID=0、
  fps 164.8）。🔴 判据须读 **`.log.err`**（引擎只写 stderr，stdout 恒 0 字节 —— 以前它读的是空
  文件，见 §21.51）。
...
- 🧊 **HUD 磨砂玻璃**（#19 已落地 `4dd3788`）：**只有「本帧 HUD 里有 `Quad::glass`」时**才把 HUD 挪到
  overlay pass，并在主 pass 后做一次 `交换链→(LINEAR)→320×200` blit 当模糊图（8×8 盒式平均）
  ——**游戏内 HUD 那条路逐字节不变**，开关 `RV3D_MENU_GLASS=0`。改 HUD 必看两条：
  **三处 HUD 绘制都要绑 `hud_glass_set`**（着色器静态引用 binding 0/1，含 PT 那条），
  且 **PT 强制 `glass=0`**（PT = 「blit PT 图 + 叠 HUD」，面板背后没有光栅画面）。
- 🔴 **⚠️ 两个同名的 `npc`，别混**（据此写下的错误结论已撤回）：**HUD 左上那行的
  `npc: I{} P{} C{} A{}`** = NPC 的**状态人数**（Idle/Patrol/Chase/Attack），**与渲染无关**
  （`I1255 P0 C0 A0` 是"1255 个 Idle"，不是 npc=1255）；**perf 日志里的 `npc=`** =
  **箱体实例数**（`last_npc_box_near + last_npc_box_far`）。
  **排查"某物有没有被画"先看这两个计数再看图**，拿 HUD 那个数当实例数会得出完全错误的结论。
...
- `MAX_LOOK_DELTA_PX = 512`（绝对位置单次位移上限，超过视为光标传送）/ `MAX_RAW_LOOK_DELTA = 1024`。

### ⭐ 无焦点视角注入配方（实测标定，误差 0.3%）

`scripts/pm_play.ps1` 已实现下面四条，**照抄即可，别再重推**。缺任何一条都会静默失效：

1. **必须用 `PostMessage`**（`SendInput` 喂前台队列，见上）。
2. **每步前重按左键**（`WM_LBUTTONDOWN`）**紧接着**投移动：否则 `WM_MOUSELEAVE` → `CursorLeft`
   → `dragging=false` ⇒ **移动进日志但视角被跳过**。
3. **每步用「窗口中心 + step」，不累加**（拖拽路径每事件重设 `last_cursor` 为中心）；步长 400、
   连续两次差 1px（否则被位置去重丢掉）。
4. **步间隔 > 150ms（用 300ms）**：`recenter_pending_until` 的 150ms 窗口会吞 `CursorMoved`。

- **实测标定**：1200px（3×400）→ **-170.30°**，模型预测 -170.79°，误差 0.3%
  （模型 = `yaw -= dx * (0.0005 + sensitivity*0.002)`；`cam:` 日志单位是**度**）。
- **`SetCursorPos` 驱动真实光标走不通**：游戏收到 **0 个** `CursorMoved`。
...
- **键码一律用 winit 0.30 `KeyCode` 枚举序号**（KeyW=41 / KeyS=37 / KeyA=19 / KeyD=22 /
  ...
  `R=82 C=67 Z=90 W=87 A=65 S=83 D=68 Space=32 Tab=9 Esc=27 Shift=16`）。混用 =
...
### 分辨率
`RESOLUTIONS` 5 档含 2560x1600；显式配置非预设分辨率会回退首项；
...
- **尺寸契约是硬的**：`city.rs` 按**占地**摆放（`building_tall` 最多，实测 **132 处**）。资产越出轮廓就会插进邻居 ⇒ **外挑一律改内凹**（凹阳台 loggia 既是东欧板楼真做法，又不占轮廓）；
...
- **顶点预算**：`props: 缓冲扩容 顶点 N/2097152` ⇒ **硬容量 2^21 = 2,097,152**，加细节前先看这个数。
  ...
  `tree_oak`（372 件 × **500**）占 **38%** ⇒ 要动就动这两类，别看件数。判据 = `RV3D_PROP_STATS=1` 的
...
- 硬件门槛：最低 3300X + RX 6500 XT（RDNA2）4C8T、内存 8GB / 推荐 12GB+；推荐 8C16T 中端独显，
  最高 16C32T + RTX 40/50。详见 `docs/hardware-requirements-2026-08-11.md`。
...
- **看到"规划中"的 dead code，必须回答"那它为什么没被接线"，不许加 `#[allow]` 了事。**
  现状：**全仓 109 处**，**绝大多数是有注释的"预留接口"**，属诚实预留不是藏问题。
...
# LLM 战术指挥会战（红蓝 128v128，实测 14 条命令全被采纳）
...
- **`scripts/*.ps1` 的每一行都不许以非 ASCII 字节结尾**（判据 = 测试
  ...
  若这行是注释，**下一行代码被静默注释掉**（2026-09-26 一次审计命中 5 个脚本、各丢 1~4 行真代码 ——
  如 `compile_pt.ps1` 丢了 `$ErrorActionPreference='Stop'`；明细见 §21.48）。
...
- **不变式（有测试钉死）**：出生环（40–80m）上每个可站立点都必须落在**玩家可达域**里 ——
  ...
  🔴 **出生点必须吸附**：`Game::nearest_in_component` 把每个出生点收进目标连通域（波次 = 玩家所在域；
...
7. **D12 士兵近距观感**：`soldier.glb` 实例化绘制；🔴 阵营色 = 队色 × `tint.w = 6.0`。**仍缺**骨骼动画与两套队色变体（`docs/HANDOFF-soldier.md`）。
...
14. **恒真的断言等于没写**（`npcs.len() >= 0`）；回归测试把**错误结论**固化下来也发生过。
...
27. **🔴 先确认你的测量工具测的是你以为的东西**（一个会话为此栽了六次）。**判据：任何量化结论之前，先用一个"必然能测出差异"的已知变化验一次工具**（2026-09-26 同形：见教训 45）。
...
40. **🔴 "某个面没画出来"先查绕序/背面剔除，再查几何参数**（代价 = 两天）：本管线水平面与竖直面的正面约定相反，立方体顶/底面与圆柱盖长期反绕 ⇒ 顶面恒被剔除，一个绕序 bug 伪装成"建模/烘焙"。**⇒ 判据 `horizontal_winding_tests`；探针 = 可疑面涂不可能色。**
...
43. **🔴 帧率口径：`fps` 必须是「窗口内帧数 / 窗口时长」**，不能是"某一帧的 `1/dt`"（同行 `frame_us` 是**另一帧**的耗时 ⇒ 自相矛盾；同一二进制两次能"差 48%"，我据此写过**错误的 +58%**，见 §21.36/§21.37）。**⇒ 先跑 `scripts\aa_probe.ps1 -Runs 3` / `ab_pair.ps1` 量噪声底，小于它不算数。⚠️ **噪声底不是常数**：重负载 0.2%、轻负载 2.5~3.0% ⇒ 每轮现场量（§21.40(b)）。**
...
45. **🔴 成本地图（任何"关掉某个东西"的对照实验）必须放「物理上必然更快」的正对照臂，
  且各臂要在**同一轮内轮转**，不许一个臂连跑几对再换下一个。**
  2026-09-26 两次实测：① `RV3D_NO_PROPS=1` **−17%**（少画 246k 三角形却更慢）、
  `NO_MARKERS` −5.6%、`NO_TERRAIN_FIELD` −15% —— 物理上不可能（§21.27 同臂是 **+37.7%**）；
  机理：外部干扰周期（分钟级）**远长于**交替周期（25 秒）。② 改成"每臂 2 对、按臂串行"再撞一次：
  控制臂 **+5.4%（通过）**，同批 `NO_DECALS` 却 **−13.4%**、`NO_MARKERS` −6.5% ——
  干扰（用户 Edge 常驻 ≈1.3 核、CPU 40%）**在批次中途才起来**。
  **⇒ ①控制臂不快 ⇒ 整批作废；②控制臂快也证明不了其余臂**（它只证明自己那一段没漂）。
  动手前后都要看 `Get-Process msedge` 与 CPU 负载；有效数字只认**同轮轮转**出来的那批。
46. **🔴 审计/闸门工具必须有第三种结局：「没跑成」。** 扫描面 = 0 不算通过、读不到输入要
  fail-closed、**空日志也不算通过**（一天在 7 个工具/驱动器里各抓到一处：`history_secret_audit`
  在非仓库目录里打"没有命中"、`commit_guard::staged_blob` 读失败当空文件、**`survive_pm` 的判据
  整整读了一份空文件** —— 见 §21.51）。**⇒ 先问：它扫到 0 个的时候会说什么？** 约定
