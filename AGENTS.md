...
17. **`survive` 5 波真机：第 1 波已清（2026-09-25），2..5 波未通**（`RV3D_MAP=assets/maps/defense_line.toml`
    ...
    核显实测（`RV3D_GPU=igpu`）：**`waves cleared: ['1']`**、波间补给触发、进入 wave 2 打到剩 3–5 只、
    `kills/shots 9/124`、`VUID=0 panics=0 device_lost=0`；冒烟（程序化城市）`ALL-OK`。
    🔴 第五次修复（`b7a3639`）= **出生点收口**：两军出生点曾落在 9 格 / 2 格的小连通域里 ⇒ 永远走不到
    一起，现由 `nearest_in_component` 吸附（判据见铁律 H）。**剩下的瓶颈在 harness 枪法**：
    ① 相机 yaw 累积到 −600° 以上而 `aim` 仍判「已收敛」；② 残局 NPC 反复进出 `Attack`、`stand` 行每几秒
    刷新 ⇒ 瞄旧行打空、且刷新会重置尝试预算。口径：`RV3D_INVINCIBLE=1`；**失败分支（玩家阵亡）仍只有单测覆盖**。
