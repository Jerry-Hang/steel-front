...
### 红蓝阵营不对称：**已定案 —— 是地图几何，不是队伍/AI**（2026-09-14）

**结论：谁在 −X 半场谁占优。** 与队伍、AI、人数、角色都无关。
原先"红方恒胜"的说法**不成立** —— 那是单次短样本的假象。

**方法**：`scripts/ab_reps.ps1` 每臂重复 4 次；`RV3D_SWAP_SIDES=1` 只交换两个出生半场
（不动队伍/人数/角色，是干净的单变量对照）。`ai:` 日志的 `red=`/`blue=` 首末差算伤亡，
`blue margin = 红损 − 蓝损`（正数 = 蓝方占优）。

| 臂 | n | 均值 | 最小 | 最大 | 极差 |
|---|---|---|---|---|---|
| baseline（红 +X / 蓝 −X） | 4 | **+9.8** | −1 | 15 | 16 |
| swapped（红 −X / 蓝 +X） | 4 | **−6.0** | −12 | 0 | 12 |

**臂内极差均值 14.0，臂间差 15.8 ⇒ 效应刚好超过噪声**（判据取自本文档自己写的规则）。
**⇒ −X 半场占优**：谁在那儿谁赢，两队都试过。

⚠️ **别把 +9.8 读成"蓝方强"** —— 那是**位置**效应；蓝方在 baseline 里恰好占了 −X。

**下一步 lead**：`city.rs` 的程序化城市**不是镜像对称**的（`ground_zone` / 街区角色 /
`push_out_of_obstacle` 都按绝对坐标走）。要找的是"−X 半场有什么 +X 没有"。
现成工具：`RV3D_DUMP_NEAR=<米>` 逐件打印相机附近的 marker 种类与尺寸。

🔴 **定案过程本身要记住**：我先做单次 A/B，据此写下"赢家跟着队伍走"**并入了库**；
同一条命令再跑一次就得到相反符号（+51 / +11）⇒ **结论撤回**。之后 baseline 六次采样
（−22 / −1 / +11 / +14 / +15 / +21，极差 43 点）才看清"红方恒胜"本身就是噪声。
**⇒ 单次 run 在这个场景连符号都不保证**（教训 24 / 27）。
