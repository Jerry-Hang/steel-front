...
30. **改回源码要用编辑器工具或 `git checkout --`**，不要过 PowerShell 字符串（会留残差）。这是教训 7 的延伸。
    ⚠️ **2026-09-15 又栽两次**：本 shell 的 `ReadAllText` 按 **GBK** 解码（65918 B 的文件只读出 38703 字符），
    于是所有针对中文的字符串替换**全部静默打不中**（报"锚点未命中"）；而我按"读到的行号"去做删除操作时，
    **行号是错的，删掉了别处的 9 行**（AGENTS.md 被改坏，靠 `git checkout --` 恢复）。
