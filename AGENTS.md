...
**最终实验（`scripts/ab_reps.ps1 -Reps 20`，40 局，每局 120 s）：**

| 臂 | n | 均值 | 最小 | 最大 | 极差 |
|---|---|---|---|---|---|
| baseline（红 +X / 蓝 −X） | 20 | **+12.8** | 0 | 29 | 29 |
| swapped（红 −X / 蓝 +X） | 20 | **−10.3** | −35 | 19 | **54** |

**臂内极差均值 41.5，臂间差 23.1 ⇒ 效应未超过噪声**（判据取自本文档自己写的规则）。
`blue margin = 红损 − 蓝损`（正数 = 蓝方占优）。

**⇒ 正确读法（三层，别只取第一层）：**
1. **方向是一致的**：两个臂的均值分居零的两侧（+12.8 / −10.3），且与 8 轮那次
   （+16.5 / −6.3）同向 ⇒ **可能真有一个约 20 点的位置倾向**。
2. **但单局噪声更大**：swapped 臂极差 **54 点**（−35..+19），臂内的离散**接近臂间差的两倍**。
3. ⇒ **任何"单次对撞谁赢"的观察都不构成证据** —— 这正是本文档教训 24 / 27 的形态。

**⇒ 因此"红方恒胜"与"−X 半场占优"两个说法都撤回**：前者是单次短样本的假象，
后者是 8 轮样本下的边界判断，**20 轮把它降级为"方向一致但未获确证"**。

**四条静态测量全部对称**（这一半的否证是扎实的，每条都有留在仓里的可复现测试）：

| 测量（测试） | 结果 |
|---|---|
| `dump_half_map_asymmetry` | 掩体占地 **0.999**（障碍 596/585、道具 288/288） |
| `dump_spawn_ring_blocking` | 出生环被挡 **39.1% / 39.4%** ⇒ **0.992** |
| (角度, 半径) 配对 | 角度均值最大差 **0.6°** |
| 平均出生半径 | **两侧完全相同**（174.00 m） |

**⇒ 可以划掉的解释**：掩体分布、出生点可站立性、出生几何配对、角色分配
（`role_for(i,1,…)` 两侧同 `i` ⇒ 镜像对称）。

**⇒ 若还要继续，先解决"测不准"，别再找机制**：当前每局方差 ~±30 点，
这个量级下找成因是给噪声编故事。可选：把每局拉长到 190 s 以上（原报告的
101/102/102/103 就是 190 s 的，比 120 s 稳定）、或找确定性种子
（AI 时序/索敌依赖浮点累积与线程调度，先确认它到底确不确定）。
