...
| 维度 | 状态 |
|---|---|
| 单元测试 | **389 passed / 0 警告**（dead-code=0） |
| 玩法冒烟 | ALL-OK（kills≥1、VUID=0、fps≥120） |
| 渲染主路径 | **VK_EXT_mesh_shader 网格着色器**（RTX 5060 真机启用） |
| 武器系统 | **35 把现代枪械已实现**（V3.0 数据表：初速/子弹下坠/散射MOA/距离衰减/部位倍率分段/开火模式/ADS） |
| 弹道 | 投射物弹道：**重力下坠 9.8m/s² + MOA 散射**（开镜缩至 30%）+ 高速弹线段碰撞（不穿掩体）；普通子弹过期/撞障碍不爆炸 |
| 大战场 | 默认 **红 64 vs 蓝 63+玩家（64v64，127 NPC）**，人形士兵（头/四肢/脚，步态动画）；RV3D_STRESS_AI=0 恢复波次模式 |
| 战术系统 | 友军伤害关闭（玩家弹穿过友军）、障碍物不可被枪械打爆（手榴弹可摧毁）、NPC 被障碍遮挡不渲染（无隔墙透视）、伤害飘字同值合并且最多 3 条滚动 |
| 小地图 | 右上角实时战术地图：地形高度灰度、障碍物按种类配色、红蓝单位点、玩家朝上旋转、500m 比例尺 |
...
| 4K 高画质 | 13700K 或 7900X | RTX 4070 Super或 RX 9070 GRE | 32GB+ |
