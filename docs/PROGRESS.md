...
>
> **本文件 340 KB / 6185 行，不要通读。** 先读这一节，再按关键词往下搜。
>
> ### 当前基线
> `cargo test --release` **503 passed / 0 failed / 0 警告**（2026-09-19）；
> 端到端冒烟 `scripts/run_smoke_pm.ps1` → **ALL-OK**（`vuid==0 && panics==0 && killed>=1`，**无 fps 门槛**）；
> `RV3D_VALIDATION=1` 跑一整轮只剩 #23 那条层侧误报（5 次交换链创建 = 5 条报文）。
> `AGENTS.md` **~64.2 KB** —— 仍超 <48KB 目标，🔴 **距 65,536 B 硬上限只剩 ~0.7 KB（按磁盘 CRLF 计）：下次加料必须先删旧料**。
>
