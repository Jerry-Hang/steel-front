...
| HESCO 坡脚石"弹珠漂浮" | 扁球底顶点只到 +0.007，地面平面在 -0.05，17cm 石头悬 5cm | 中心 z 0.09→0.02，入地半埋（min_z -0.068） |
| 沙袋墙整堵浮空 7cm（§20.4"扁八边形垫"存疑结案） | 底层中心 0.15，扁球底 +0.018，加地面 -0.05 ⇒ 0.7m 矮墙悬 6.8cm，远读"垫子漂地" | 底层 0.08，min_z -0.052 恰切地平面；包长/步距复核为本来就咬合（见下"误修回退"） |
...
**立面均值系数——负结果结案**（终止了反复停摆的代理任务，手算）：
marker 建筑立面窗带暗化 = 0.22 × 窗带覆盖率 (2.73−0.62)/3.15=0.670 × 竖梃修正
(1−0.7×0.14)=0.902 ⇒ 3m 以上 ×0.867；面积加权（裙层 3m、屋顶、玻璃面拉回 1.0）≈0.90。
**但 §19.4 同日 A/B 实测 bldg 区域 PT/raster 亮度比已是 ×1.00**——0.133 的暗化差已被
其他项（PT 无雾、色调曲线）吸收，再乘 0.90 反而制造 10% 新偏差。`pt_albedo_of` 不改。
教训：**经验比值优先于理论推导；系数表类改动必须先有同日 A/B 基线**。
