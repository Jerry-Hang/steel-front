...
### 21.73 把「尺子」也审一遍：三件套的两个真缺陷 + 无 GPU 自检法（`96a0a5e` 系列）

**为什么审尺子**：§21.72 的主题是「失败被静默抹平」，而**性能尺子本身**正是最不能撒谎的工具
（教训 43/45 都出自它的口径问题）。显存被占着跑不了游戏，正好可以把 `perf_run` / `ab_pair` /
`aa_probe` 当**纯代码**审一遍 —— 结果抓到两个真缺陷，且都能在**没有 GPU、没有游戏进程**的条件下复现。

| 缺陷 | 症状（此前） | 修法 | 判据 |
|---|---|---|---|
| `perf_run.ps1` 的日志发现用 `-Filter "perf_*.log"`，而**它自己的 stdout 重定向就叫 `logs\perf_run.log`** | 该文件恰好最新时，脚本分析的是**自己那份 stdout**，解析出 0 行，然后报「only 0 sample row(s) parsed; the run was too short」—— **读错文件却怪这一次跑得短**（误导排查方向） | 发现时按文件名排除 `perf_run.log` | 自检 T1/T2 直接命中（修前两条都报 0 行） |
| `perf_run.ps1` 的「稳态窗口」退化路径**静默**：`t >= 3s` 的样本不足 3 个时**偷偷改用全部样本**，标题却仍写「steady-state = t >= 3s」 | 一次早退/短跑的数字与正常测量**长得一模一样**，`ab_pair` 会照单收下当成一个有效臂 | 打印**实际使用的规则** + 走退化路径时**退 2**（0=可用 / 1=没数据 / 2=没稳态窗口） | T1（5 行全 `t<3`）⇒ WARNING + **exit 2**；T2（8 行 `t=1..8`）⇒ `steady-state = t >= 3s, n=6` + **exit 0** |

**连带的协议加固**（同一个「尺子不许撒谎」主题）：

- `ab_pair.ps1` 以前**完全不看 `perf_run` 的退出码**，只找不找得到那行 fps —— 现在非 0 一律
  判该臂不可用；并统计「完成 / 请求 / 跳过」的配对数，**批次不全时退 2**（这正是教训 45
  「n ≥5 对」被悄悄违反的地方）。
- `ab_pair.ps1` 现在**逐对轮转顺序**（奇数对 A→B、偶数对 B→A，即 ABBA）：臂的配对差定义不变，
  但「一对里的第一次跑系统性偏快/偏慢」这类残留偏差会对消 —— ⚠️ **本次没有实测它的收益**
  （显存窗口关着），标注为协议加固而不是已测改进。
- `aa_probe.ps1` 的文件头写着「有一次没解析出来就退 1」，代码却只在**不足 2 次**时才退 1 ⇒
  6 次里坏 4 次照样打一行「噪声底」并退 0。现在按文件头执行：全可用退 0、部分可用退 2
  （并打印「K usable of M requested」）、不足 2 次退 1。

**🔴 无 GPU 也能做的尺子自检（`logs/ps_selftest/`，配方留档）**：
把三个脚本复制到一个沙箱目录、只改 `$repo` 指向沙箱；用**桩 `perf_run.ps1`**（打印与真脚本同格式的
两行、按 `$env:STUB_RC` / `$env:STUB_FAIL_EVERY` 注入退出码）喂 `ab_pair` / `aa_probe`；
再用**合成 perf 日志**（`mklog.ps1`，12 列与 `perf_log.rs` 对齐）+ 一个「秒退」的假 exe
喂**真** `perf_run`。实测结果：

| 用例 | 期望 | 实测 |
|---|---|---|
| `ab_pair -Pairs 4`（全成功） | 4/4 对、顺序 `AB BA AB BA`、退 0 | ✅（且桩的抖动在臂间对消 ⇒ 中位差 0.00、符号 2/4 —— **负对照**：尺子不会凭空造出效应） |
| `ab_pair` 每第 4 次调用失败 | 跳过 2 对、退 **2** | ✅（修前会把这 2 对当成有效数据） |
| `aa_probe -Runs 4`（2 次不可用） | 「2 usable of 4」、退 **2** | ✅ |
| `aa_probe -Runs 4`（全不可用） | 退 **1** | ✅ |
| 真 `perf_run` + 5 行 `t<3` 的日志 | WARNING + 退 **2** | ✅ |
| 真 `perf_run` + 8 行 `t=1..8` 的日志 | steady-state n=6 + 退 **0** | ✅ |

**顺带把 AGENTS.md 模块地图的行数全部复测了一遍**（它自己写着「陈旧数字和错的结论一样有害」）：
...
### 21.77 军情的两个数自相矛盾：一个尸体 + 一次过期快照（`ba79fda` + `70598bd`；判据工具入库）

**怎么找到的**：给 §21.76 的修复做真机验收时，我把「军情口径」写成一条**可重跑的不变式** ——
`击杀 + Σ连强度 == 编制`（红 128 / 蓝 127），逐行扫会战日志。第一版扫出来 **66 行里 6 行超出 1~3 人**
（`+1 +1 +3 +1 +1 +1`）。注意：这是**为了验收上一个修复才写的判据**，结果它自己抓出两个新缺陷。

**两个原因，方向相同（都让和偏大）**：

| # | 原因 | 证据 | 修法 |
|---|---|---|---|
| 1 | 连报告按「在 `npcs` 里」数人 ⇒ 本帧刚阵亡、还没被清场的**尸体同帧既算活人又算阵亡** | 红测 `company_report_counts_only_the_living`（造出那个同帧状态；去掉修复后实测 `left 64 / right 63`） | 存活者列表加 `hp > 0.0` |
| 2 | `kills` 在 tick 判定**之前**赋值（每帧都新），而连报告要等 0.5s 的节拍 ⇒ 日志行里「击杀」是这一帧的、强度是**上一 tick 的快照** | 临时埋点（每帧打 `k/alive/corpse/missing/rep`）：**33 974 帧里 `k+alive+corpse == 编制` 100% 成立**，而 `rep != alive` 出现在 4 012 帧，差值恰好是"这一 tick 刚死的人" | `self.kills = kills` 移进 tick 之后 ⇒ 两个数同一次刷新 |

只修 #1 时日志里仍有 **9/66** 行超 1~3 人（说明 #2 是独立存在的第二个原因）；两个都修之后
**60/60 行全部成立**（`python tools/battle_tally_check.py logs/llmbattle.log.err` ⇒ exit 0）。

**判据工具入库**：`tools/battle_tally_check.py`（用法 `python tools/battle_tally_check.py logs/<tag>.log.err`）。
退出码按「扫描器要有第三种结局」：**0 = 扫过且干净 / 1 = 有命中 / 2 = 根本没跑成**
（无文件、无编制行、无军情行都算 2）。⚠️ 写它时把 杀(U+6740) 打成了 伤(U+4F24) ⇒ 一行都匹配不上，
而它报的是 **`did not run`** 而不是 `OK` —— 那条约定当场挡住了一次假通过。
...
（判据只认 `build`）；CJK 字模闸门 OK；`.ps1` 行尾闸门 OK；工作树干净、全部已推送。

**这一轮动过什么（17 个 commit）**：`bfd79e7`/`c0ea1f9`/`2752ea8`/`f51d8df`（AGENTS 体量与判据），
`7877855`（编制尾数并入末位）、`44c7464`（重组阈值分母）、`519f576`（0 警告闸门回归）、
`8e913aa`（阵亡计数漏主路径）、`2c0fa62`（注释残留）、`ba79fda`（军情只数活人 + 判据工具）、
`70598bd`（军情快照同源）、`8a84336`/`3c59bed`/`f4b3240`（§21.75–21.78）、
`9a177e3`（重组可达）、`3a1f0ff`（未结案 27 结案）。
**共同主题**：一个数字或一个口径与事实不符 —— 全部是"不崩、不报错、只是让人相信一个错的战场"。

**没跑成的（诚实记账，别当成已验）**：
1. **收尾冒烟**：GPU-BUSY（外部 4344 MiB）。下一轮空闲窗口补跑。
2. **真机上看到 `态势Regroup`**：150 秒那一局双方伤亡过半时战果都已 ≥8 ⇒ 规则上不该重组
   （可达性由单测钉死）。要看真机事件，得跑**单方面挨打**的场景。
3. **n≥5 对的轮转成本地图**：今晚 GPU 一直被占（用户在用），没抢。

**下一步（按价值排序，都带 lead）**：
1. **联网双进程真机验证**（`AGENTS.md` #13）：NAT 打洞注册/解析已接线，缺真机两台/两进程；
   顺手验 §21.74 的据点进度所有权（联网路径才达）。
2. **PT 与光栅同屏叠加**（#11）：lead = 像素重投影复用或运动自适应 spp；`signature()` 分层
   （~0.5m/~3°/~0.01）**勿回退到 1mm**。
3. **士兵骨骼动画**（#7，`docs/HANDOFF-soldier.md`）：`soldier.glb` 已实例化绘制，缺动画。
4. **成本地图**（教训 45 三件套已就绪）：GPU 空闲窗口跑 n≥5 对轮转，先量 A/A 底噪。
5. **README 与实现的对齐**：本轮已把 README 的进度段改成"指向 AGENTS/PROGRESS"，
   并修掉三处坏指针（`publish.ps1` 不存在、`gen_props.py` 已被取代、废弃的 SendInput 冒烟脚本）。

**交接给下一个人（三条最容易踩的）**：
- 🔴 **改 `scripts/*.ps1` 后行尾必须是 ASCII**，且 `cargo test` 与 `cargo build` 的警告口径不同
  （`test` 会漏报）。
- 🔴 **改军情/编制相关代码后跑 `python tools/battle_tally_check.py logs/llmbattle.log.err`**：
  两条不变式（`阵亡 + Σ强度 == 编制`、`本营战果 == 敌方阵亡`）任一红都说明口径错了。
