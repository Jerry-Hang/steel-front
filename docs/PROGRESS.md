...
>
> **本文件 340 KB / 6165 行，不要通读。** 先读这一节，再按关键词往下搜。
>
> ### 当前基线
> `cargo test --release` **502 passed / 0 failed / 0 警告**（2026-09-19）；
...
> 下午～傍晚（§9~§11）：我引入的 ±8 长椅双环回归被"长板"特写钉死并删除（判据 `benches_per_plaza_is_exactly_one_ring`）；D4 亮带用数值闭案；`tools/patrol.py` 落成 10 机位数值巡检工具；花坛灌木"裙边"改 `bush(base)` 圆顶判据（`planter_bushes_are_clean_domed`，502 绿）；商铺骑楼补拍确认 PASS（第一次拍的是背面——**断言构件没渲染前先确认相机在正面**）。
>
