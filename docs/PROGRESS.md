...
>
> **本文件 340 KB / 6252 行，不要通读。** 先读这一节，再按关键词往下搜。
>
> ### 当前基线
...
- **PT 阻塞项降级**：用 `target/pt_home/.steel_front.cfg`（scratch HOME，不碰用户配置）
  ...
  **09-03 定案的 0xC0000005 在当前驱动/SDK（1.4.357）下不复现**——当年"源码逐字节
  回退 35a 仍崩"的结论没作废，变的是环境。默认仍是关（一次 70s 探针不足以翻默认），
  但"PT 同屏叠加"从崩溃问题降级成下面的曝光校准问题。
- **PT live 表面过曝 ~2×**：同机位分区对照（pt1_b vs bat1_b，`fly:0,1.7,30:180,2`）：
  天空 ×1.02 一致，路面 ×2.14（92→197）、楼体 ×1.91、树冠 ×1.90。沥青 albedo 0.115
  在参考曝光下应落 ≈97 灰阶，实测 197 ⇒ 入射亮度约 5×（线性）。PT-VIEW 路径有
  `exposure: 0.5`，live 合成路径疑似没吃到等价的曝光/色调映射 ⇒ **下一候选专项：
  PT live 曝光统一**（改动前先看 ray_tracer 的 uniform 结构，可能不是单行）。
