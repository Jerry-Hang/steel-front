...
> | ④ 人物建模 | 🔶 **已定案，需单独立项** | 18 段 + 预算守卫 + 单测；**已修五处**（持枪姿态 / 逐段明暗 / 头背心重叠 / 脸盔明暗反转 / **躯干收窄至真人比例，实宽 0.72→0.48m——实机特写确认手臂终于与躯干分离**）。**近距取证已打通**（RV3D_NPC_CAM+RV3D_NPC_POS+RV3D_NO_PROPS，剔除已在代码里修好）。**结论：程序化箱体在近距读不出人形 ⇒ 需要 Blender 做士兵 GLB**（会动 NPC 实例系统） |
