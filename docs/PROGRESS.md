...
> | ⑦ 骨骼 + 烘焙 | 🔶 **并非"未开始"**（第 96 轮查明） | **烘焙已有一半**：地面纹理的 AO/静态天光已烘焙在 `procedural.rs`（地形高度场凹度遮蔽；**不在 `lighting.rs`** —— 后者是纯运行时光照）。**地形之外的几何烘焙不进顶点色**（marker/NPC 顶点色被白化，只能走 tint）⇒ 只能靠**逐段/逐类明暗**（士兵已做）。**骨骼绑定完全没有**：NPC 是"每段一实例 + 按相位摆动"的程序化动画 |
