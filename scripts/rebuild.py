# -*- coding: utf-8 -*-
import io
s = io.open(r'screenshots\\README_old.md', encoding='utf-8').read()
# ① 完整配置表更新：最低档 CPU → 3300X / i3-11代；表下加说明
...
if old_row in s:
    ...
    print('min row updated')
# ② 在"## 硬件推荐配置"前插入"最低配置（2026-09-01 更新）"小节（CPU 明确 + PT 权限）
...
if "## 硬件推荐配置" in s:
    ...
    print('min section inserted')
io.open(r'screenshots\\README_new.md', 'w', encoding='utf-8', newline='\n').write(s)
print('saved README_new', len(s))
