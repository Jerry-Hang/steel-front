...
fn plaza(c: &mut City, cx: f32, cz: f32, monument: bool) {
        ...
        .enumerate()
    {
        ...
        //   所以正确做法不是调平，而是**根本不做出空洞**：两层实心盒叠起来，
        //   土台面比石台再高 4cm ⇒ 任何机位都只看到一个"石座 + 土面 + 植物"的实心台。
        c.push(Part::new(ObstacleKind::Building, ax, az, 3.4, 3.4, UNDER_GROUND, 0.30, GRANITE));
        c.push(Part::new(ObstacleKind::Building, ax, az, 2.9, 2.9, UNDER_GROUND, 0.34, SOIL_DARK));
