...
fn plaza(c: &mut City, cx: f32, cz: f32, monument: bool) {
        ...
        .enumerate()
    {
        ...
        // 花坛：**实心台 + 顶面是土 + 灌木从土里长出来**。
        //
        // 🔴 三版失败记录，根因是同一个：**只要花坛有"内部空洞"，它就一定会被读成坑。**
        //   1. `4×4×0.62` 整块石头 + 灌木长在石头顶上 ⇒ 读作"广场上一块被遗弃的混凝土墩"
        //      （`v6_bush_b.png`）。
        //   2. 石框 0.42 + 土面 0.30 ⇒ 越框可见 12cm 台阶 + 一整片暗棕土面，
        //      读作"塌了的木箱坑"（`v10_tree_b.png`）。
        //   3. 石框 0.42 + 土面 0.42（齐平）⇒ **仍然像坑**：相机离框 1.3m、眼高 1.5m，
        //      俯视时 0.37m 的内壁立面在画面里就是一道"往下掉"的墙，
        //      而齐平的土面角在 2.1m 处正好读成一块棕色碎板（`v14/v15/v16_same_b.png`
        //      三张同机位复拍完全一致 —— 这也是本条判据的来源）。
        //   判据：**"内壁可见高度"就是坑的成因，与土面是否齐平无关。**
        //
        // 🔴🔴 最后定案：**那块"土"本身就是缺陷，跟齐平与否无关。**
        //   2.9m 的棕色土台压在 3.4m 的灰色石台上、四周各缩进 0.25m、顶面高出 4cm。
        //   从 2-3m 俯视 ⇒ 看到的是"一个灰托盘 + 托盘里一只棕色盒子"，
        //   而那只盒子的**两个侧面 + 顶面**在平着色下正好拼成一把椅子的
        //   座板 + 靠背 + 腿 —— 这就是我追了很多轮机位复拍的"坑里一张塌掉的长椅"。
        //   它不是长椅、不是尸体、不是透视错觉，**是我自己多放的一个盒子**。
        //   ⇒ 判据：给地面物件加"材质贴片"式的第二层薄盒前，先问
        //      **它的侧面会不会在低机位被读成一件独立物体**。会，就不要加。
        //   现在只留一层石台，灌木的团块直接盖住台顶（冠幅 2.0r ≈ 台顶 3.4m 的内接范围）。
        c.push(Part::new(ObstacleKind::Building, ax, az, 3.4, 3.4, UNDER_GROUND, 0.34, GRANITE));
        // 灌木一律走 [`bush`]：四团明显分离的 lobes。
    ...
    for side in [-1.0f32, 1.0] {
        for k in 0..6i32 {
            ...
            // 柱础与柱头都做**方压盘**，不做同径圆盘。
            // 🔴 圆盘从下方看只有"一个朝下的面"，而本引擎法线来自屏幕导数 ⇒ 朝地面的面
            //    NdotL=0、只剩环境光 ⇒ 一块纯暗色的圆盘面，实机读作"一根管子的开口"
            //    （`v12_planter_b.png` 两根柱的头顶）。第一版改成 1.06m 方盘仍不够：
            //    它只比柱身（0.62m）宽 22cm、又只有 0.35m 高，侧立面在 5m 外不到 3 像素，
            //    于是看到的还是那块暗底。现在 1.40m 见方 × 0.50m 高 —— 侧立面够宽、
            //    且明显出挑，一眼是"柱子的头"。
            // ⚠ 颜色与形状同等重要：压顶的朝下面 NdotL=0，只剩环境光，用深色 GRANITE(0.34)
            //   时它就是"一块黑底 + 一圈受亮的侧边"，仍然读作管口。换成与檐梁同系的
            //   CONCRETE(0.56) 后，柱头与梁底连成一片受光的整体。
            c.deco(Part::new(ObstacleKind::Block, px, pz, 1.02, 1.02, UNDER_GROUND, 0.34, GRANITE));
            c.deco(Part::new(ObstacleKind::Block, px, pz, 1.40, 1.40, 4.45, 4.95, CONCRETE));
        }
...
/// 一条长椅（`along` = 长边沿 x 轴）。
///
/// 🔴 板件厚度是**可读性**问题，不是结构问题。旧版座板 0.42..0.54 = **12cm**，
/// 从正面看几乎是一条线 ⇒ 实机（`v7_bush_b.png` 正中）只剩"一块悬空的靠背板 +
/// 两条深色腿"，读作一张塌掉的长椅，而不是一张完整的长椅。本引擎无法线槽位、
/// 纯平着色，**任何小于 ~15cm 的板在 15m 外都会消失**，所以家具的板厚要按
/// "能被看见"来定，不是按真实木工尺寸来定。
/// 现在：座板 20cm、靠背 48cm 高（真人靠背也就这个数，旧版 64cm 反而像围栏）、
/// 腿加粗到 0.28 × 0.60 并坐到座板底下（不再与座面等高相切）。
fn bench(c: &mut City, x: f32, z: f32, along: bool) {
    let (w, d) = if along { (2.4, 0.62) } else { (0.62, 2.4) };
    c.push(Part::new(ObstacleKind::Block, x, z, w, d, 0.34, 0.54, WOOD_BENCH));
    let (bw, bd) = if along { (w, 0.22) } else { (0.22, d) };
    let (bx, bz) = if along { (x, z - 0.20) } else { (x - 0.20, z) };
    c.push(Part::new(ObstacleKind::Block, bx, bz, bw, bd, 0.54, 1.02, WOOD_BENCH));
    for s in [-1.0f32, 1.0] {
        let (lx, lz) = if along { (x + s * (w * 0.5 - 0.28), z) } else { (x, z + s * (d * 0.5 - 0.28)) };
        c.push(Part::new(ObstacleKind::Block, lx, lz, 0.28, 0.56, UNDER_GROUND, 0.36, DARK));
    }
}
...
/// 灌木：**四团明显分离的 lobes**，不是一堆互相穿插的球。
///
/// 三个版本依次踩坑，都记下来：
/// 1. **两团几乎同心**（原始版）：平着色下读作**一块绿色巨石**（`v5_bush_b.png`）。
/// 2. **五团深叠**（第一版改法，外圈 0.35..0.44r 全小于中心 0.52r）：外球面与中心球面
///    **近乎相切**，相切处的两条曲面法线差极大 ⇒ 平着色把每条交线画成一道深色 V 槽，
///    轮廓上还挤出三角形尖角，实机像**揉皱的金属皮**（`v14_same_b.png` 右侧那一团），
///    比巨石更难看 —— 这是"改了但更糟"，必须回退重想。
/// 3. **现在**：中心一团 + 三团成 120° 分布，偏心 0.50r 而半径 0.50r ⇒ 相邻两团只
///    相交 0.13r（**接近垂直的干净交线**），每团都完整露出轮廓。四团 = 一个有起伏的
///    灌木冠，而不是一个球，也不是一堆碎片。
///
