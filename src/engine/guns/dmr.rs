...
pub fn svd12() -> crate::engine::guns::GunMesh {
    ...
    let parts = vec![
        // 1 粗枪管
        (t(0.0, 0.045, 0.325) * rz(), cylinder(0.018, 0.45, 20), bright),
        // 2 枪口制退器
        (t(0.0, 0.045, 0.575) * rz(), frustum(0.022, 0.028, 0.05, 20, true), dark),
        // 3 导气管
        ...
        // 9 大瞄准镜身
        (t(0.0, 0.190, 0.05) * rz(), cylinder(0.022, 0.30, 20), deep),
        // 10 前端镜头
        (t(0.0, 0.190, 0.205) * rz(), frustum(0.027, 0.027, 0.02, 20, true), deep),
        // 11 后端镜头
        (t(0.0, 0.190, -0.105) * rz(), frustum(0.027, 0.027, 0.02, 20, true), deep),
        // 12 镜架前环
        ...
        // 15 拉机柄球
        (t(0.079, 0.120, -0.15), sphere(16, 10), deep),
        // 16 10发弹匣（略前倾）
...
pub fn m110a1() -> crate::engine::guns::GunMesh {
    ...
    let parts = vec![
        // 1 长粗枪管
        (t(0.0, 0.05, 0.31) * rz(), cylinder(0.017, 0.42, 20), bright),
        // 2 枪口制退器
        (t(0.0, 0.05, 0.545) * rz(), frustum(0.022, 0.026, 0.05, 20, true), bright),
        // 3 全长护木
        ...
        // 9 大瞄准镜身
        (t(0.0, 0.205, 0.03) * rz(), cylinder(0.022, 0.26, 20), deep),
        // 10 前端镜头
        (t(0.0, 0.205, 0.161) * rz(), frustum(0.025, 0.025, 0.018, 20, true), deep),
        // 11 后端镜头
        (t(0.0, 0.205, -0.101) * rz(), frustum(0.025, 0.025, 0.018, 20, true), deep),
        // 12 高低调节钮
...
pub fn mk14p() -> crate::engine::guns::GunMesh {
    ...
    let parts = vec![
        // 1 长粗枪管
        (t(0.0, 0.05, 0.32) * rz(), cylinder(0.017, 0.44, 20), bright),
        // 2 消焰器
        (t(0.0, 0.05, 0.57) * rz(), frustum(0.024, 0.028, 0.06, 20, true), dark),
        // 3 准星
        ...
        // 13 拉机柄头
        (t(0.099, 0.145, -0.02), sphere(16, 10), dark),
        // 14 20发直弹匣
        ...
        // 18 大瞄准镜身
        (t(0.0, 0.225, 0.05) * rz(), cylinder(0.022, 0.28, 20), deep),
        // 19 前端镜头
        (t(0.0, 0.225, 0.191) * rz(), frustum(0.027, 0.027, 0.018, 20, true), deep),
        // 20 后端镜头
        (t(0.0, 0.225, -0.091) * rz(), frustum(0.027, 0.027, 0.018, 20, true), deep),
        // 21 镜架前环
