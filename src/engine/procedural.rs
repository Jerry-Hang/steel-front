...
fn ground_material(x: f32, z: f32, seed: u32) -> [f32; 3] {
    ...
    // 明确分区配色（保证可辨识）：**干枯植被** / 沙地黄 / 石板灰
    //
    // 🔴 2026-09-13：第一项由 `[0.20, 0.50, 0.15]`（饱和草绿）改为低饱和的干枯橄榄。
    // 俯瞰取证（`RV3D_CAM=fly:0,140,80:0,50`）显示全城外围是大片鲜绿，
    // 与 AGENTS.md 的美术基调「2020s 东欧/中东战乱城镇，混凝土板楼 + **冷灰色调**」冲突。
    // 亮度关系保持不变（绿 > 沙 > 石），只是把色相从"草坪"挪到"荒芜植被"。
    let grass: [f32; 3] = [0.165, 0.185, 0.105];
...
fn city_zone_color(zone: u8, x: f32, z: f32, seed: u32) -> [f32; 3] {
    match zone {
        2 => {
            ...
            // 🔴 上面两条硬边色带在实机读作"路面上一摊一摊的黄褐色污渍"，不像标线
            //    （`v7_street_b.png`）。宽度是被分辨率逼出来的折中（本图 2 纹素/米，
            //    0.15m 的真车道线违反奈奎斯特，画出来只会摊成一条更宽的淡带），
            //    但**硬边界**不是：硬边在 mip 平均时能量集中、边缘突然，正是"一摊"的成因。
            //    改成软衰减带：中心 0.35m 满强度、到 1.1m 收敛为 0 ⇒ 视觉宽度 ≈0.8m，
            //    并且改成 lerp 而不是整条替换，保住下面的车辙亮度不再被标线打断。
            let centre = 1.0 - smoothstep(0.35, 1.1, across);
            let edge = 1.0 - smoothstep(0.0, 0.5, (across - 4.5).abs());
            if dashed && centre > 0.0 {
                c = lerp3(c, [0.168, 0.158, 0.122], centre * 0.85);
            }
            if edge > 0.0 {
                c = lerp3(c, [0.180, 0.180, 0.185], edge * 0.70);
            }
            c
        }
        3 => {
        ...
        _ => {
            // 🔴 2026-09-13：**未铺装地块由"草坪绿"改为"荒芜夯土"**。
            //
            // 原色 `[0.088, 0.225, 0.062]` 是饱和度很高的草绿。而 `ground_zone` 对
            // **所有未铺装街区**都返回 0 ⇒ 全城空地都是这抹草坪绿。俯瞰取证时一目了然：
            // 大片鲜绿，与本项目的美术基调冲突 —— AGENTS.md 写的是
            // 「2020s 当代东欧/中东战乱城镇（混凝土板楼 + 抹灰老城 + 破损，**冷灰色调**）」。
            // 战乱城镇的空地是**夯土 + 杂草 + 碎屑**，不是修剪过的草坪。
            //
            ...
            if patch > 0.35 {
                ...
                c = lerp3(c, [0.205, 0.180, 0.110], m * 0.55);
            }
            c
        }
    }
