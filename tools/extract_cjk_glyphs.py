...
THE SOURCE FONT IS NOT COMMITTED -- AND A LOOKALIKE WILL NOT DO
--------------------------------------------------------------
The checked-in table records its own provenance in its header (currently
`noto-sc-subset.otf`). That file is **not** in the repository, so the table
cannot be regenerated byte-for-byte without it. Two consequences:

* **Adding a new Chinese character to `src/` will fail the test
  `font_cjk::tests::source_cjk_codepoints_all_have_glyphs`** until the table is
  re-extracted -- and re-extraction needs that exact font. If you do not have
  it, **rewrite the comment using characters already in the table** instead.
  That is not a workaround; it is the cheap correct answer.
* **A different face is not a drop-in substitute.** Measured 2026-09-15:
  pointing this tool at the system `C:\\Windows\\Fonts\\NotoSansSC-VF.ttf`
  rewrote *every* glyph (1596 changed lines), and the variable font's default
  instance renders lighter -- `灭` came out 7 rows tall and tripped
  `font_cjk::tests::cjk_glyph_generates` ("字形过稀疏 rows=7 cols=9").
  The density assertion in that test exists precisely to catch a silent
  face swap, so **do not "fix" it by relaxing the threshold.**
