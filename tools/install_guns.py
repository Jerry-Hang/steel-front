...
KEY_MAP = {
    ...
    "sv98": "sv98",
    "svd_63_cleaned": "svd12",   # 见下面 CLEANED 的说明：源文件是产品图，必须先清理
    "vss_vintorez": "vss",
    ...
    # 对照：svd_63 没有这种反证 ⇒ 走 CLEANED 那条路（清理后改名 svd_63_cleaned）。
}

# 需要**先跑一步清理**才可用的源文件：名字 → (清理脚本, 原因)。
# 放在这里而不是只写在 SKIP 里，是为了让"为什么 svd_63 不在 guns_ext 里"有唯一答案，
# 同时给出可再生的一步步命令（2026-09-26 之前它只在 SKIP 里，等于没人能重跑）。
CLEANED = {
    "svd_63_-_dragunov": (
        "tools/blender/clean_svd_shot.py",
        "源文件是产品宣传图：含两把相差 90° 的完整枪身（侧视 + 俯视）+ 独立瞄具 + 备用弹匣 "
        "+ 散落子弹 + Wire 描边副本，prep_guns 会把它们**全部合并**（报告里 dup_warn=true，"
        "并注明合并后的长短轴无意义）。清理脚本保留侧视那一把、把瞄具**按实测的机匣顶面"
        "坐到枪上**，其余全删，然后照常 prep。产物名 svd_63_cleaned.glb → 引擎 key svd12。",
    ),
}

# 明确**不安装**的原始源文件，以及原因。留在这里是为了下次有人看到 svd_63 躺在
# guns_ext 里时，不会以为是漏装了。2026-09-26 起 svd_63 已经**不再**是"不装"，
# 而是"先清理再装" —— 见上面的 CLEANED；这里保留一条指向，免得有人按老结论找。
SKIP = {
    "svd_63_-_dragunov":
        "已结案（2026-09-26）：不再是 SKIP，见 CLEANED —— 先跑 tools/blender/clean_svd_shot.py，"
        "再 prep 成 svd_63_cleaned.glb，装为引擎 key svd12。",
}
